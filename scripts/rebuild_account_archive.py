#!/usr/bin/env python3
"""Explicit local Archive backfill; never invoked by page reads."""
...
def main():
    ...
    parser.add_argument("--cache", type=Path, help="Archive sidecar (defaults beside --db)")
    parser.add_argument(
        "--install-revisions",
        action="store_true",
        help="Install/repair local Archive revision tracking before rebuilding",
    )
    parser.add_argument(
        "--filters-json", default="{}", help="Archive filter overrides as a JSON object"
    )
    ...
    if args.cache:
        config.SPOTIFY_STATS_ARCHIVE_CACHE_PATH = str(args.cache.resolve())
