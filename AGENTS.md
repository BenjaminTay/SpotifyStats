...
- 首页组合响应使用 revision/filter 精确持久快照与语义 LKG；`warming` 只能表示旧事实正在后台更新，打开首页不得同步冷建年度报告或完整 Billboard。
- 音乐详情 `summary` 必须保持独立轻量路径，不得先构建完整详情再裁剪；实体统计首屏只加载目标范围，完整全局排名通过 `include_rank_context` 延后请求。
