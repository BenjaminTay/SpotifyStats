...
- 播放统计规则引擎 Phase C+D（2026-06-12）：
  ...
  - 2026-06-19 全栈验证与性能收口：Billboard 分段接口共享基础排名缓存，Power Score、summaries、`_add_running_metrics()` 与 `merge_consecutive_plays()` 向量化；Dashboard full 复用同一播放 DataFrame；专辑详情 source breakdown 批量查 album metadata；AI Insights 周报/月报/年度叙事/自由问答透传 `dynamic_threshold` 与 `max_merge_gap_minutes` 到计数链路，并用过滤指纹分流报告缓存；Behavior 保持全量事件分析语义，仅暴露/请求有效的 `music_only` 参数；`load_plays()` / `load_plays_for_artists()` 缓存 miss 用 `singleflight()` 去重；warmup 改为 `dynamic_threshold=True` 默认口径；390px 移动端页面级横向滚动归零；pre-commit ruff/format 收敛到 `backend/`；前端 OpenCC full 包拆为 t2cn/cn2t 子包，ECharts 默认入口改为 `LazyEChart` 按需 core 入口；账号页资源从约 250 requests / 25MB 收敛到约 91-92 requests / 7.5MB，Web Vitals lab 已采集；新增 `scripts/api_smoke_probe.py` 覆盖 91 个本地只读 GET 与 OpenAPI GET 覆盖核算，新增 `scripts/frontend_route_smoke.mjs` 覆盖 13 路由 × 2 视口并锁定 0 console error/0 横向溢出；Chat/Settings/LLM profile mutation、AI Insights 生成端点和 Import job 调度已用 contract 临时环境验证；修复 Spotify 当前播放 token refresh 写连接边界、Settings 越界配置/清翻译缓存缺表和 AI Insights 会话列表嵌套按钮问题
  ...
  - 测试基线：backend full 586 / unit 239 / contract 147；frontend 128；`npm run build`、`sh scripts/phase5_check.sh`、`.venv/bin/pre-commit run --all-files`、`.venv/bin/python scripts/api_smoke_probe.py`、`node scripts/frontend_route_smoke.mjs --viewport both --wait-ms 2500 --max-scroll-overflow 0` 通过
