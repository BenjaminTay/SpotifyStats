...
- 播放统计规则引擎 Phase C+D（2026-06-12）：
  ...
  - 测试基线：backend full 590 / unit 243 / contract 147；frontend 129；`npm run build`、`sh scripts/phase5_check.sh`、`.venv/bin/pre-commit run --all-files`、`.venv/bin/python scripts/api_smoke_probe.py`、`node scripts/frontend_route_smoke.mjs --viewport both --max-scroll-overflow 0`、`node scripts/frontend_interaction_smoke.mjs` 通过
...
# 前端 route smoke + Web Vitals lab 采样（需后端 8000 + 前端 5173 已启动）
