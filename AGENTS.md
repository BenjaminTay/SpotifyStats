...
**年度总结 V2（2026-08-12）**：`/yearly-review` 的 Desktop/Compact 自定义总结使用确定性八章 `YearlyReviewV2`：报告护照、播放/时长双榜、个人 Billboard 年度赛季、唯一时间线、关系故事、收听生活、纪录、品味迁移与完整索引；Phone presentation 继续使用 V1，官方 Wrapped 继续使用官方导入数据，三者不得互相替换。后端只读 API 为 `/api/yearly-review/available-years`、`/{year}`、`/{year}/records`；主报告与分页 records 必须共享完整过滤指纹、策略版本、display taxonomy/艺人元数据 revision 和缓存语义。月度正文只保留一条转折时间线，十二月明细进入可展开账本；播放榜与个人 Billboard 不得混名，YTD/partial/unknown 必须由 coverage 控制措辞，LLM 不参与事实生成。同比只使用真实 aligned window，工作日/周末使用观察区间自然日日均，Passport 与播放榜共享规范实体粒度；YTD 品味只比较完整季度，不足时只展示分布。公开纪录必须命中显式 renderer 并具备周期/数值证据，阶段无法证明时为空，结语不得复制开篇。schema version 与 content version 分开治理，任何统计/编排语义变化都必须提升 content version。主 JSON 未压缩预算 512 KiB、热响应 250ms、真实重算预算 30s；关系历史日期边界必须按实体类型一次聚合，禁止在逐实体循环中反复扫描/转换全历史。最终 artifact 以完整精确 cache key 压缩写入独立 `data/yearly_review_cache.db`，cache key 使用稳定播放事实 revision 而非 SQLite main/WAL 物理 mtime，不得写回统计主库或返回 stale artifact；设置变更、流式导入和启动 warmup 只负责后台预建最新年份。content v2.6 的 2023–2026 同进程真实重算为 9.80–15.43s、热响应 5.04–21.65ms，跨进程持久命中 9.75–19.71ms；公开文案、精选证据、结语去重和阶段状态由 `scripts/yearly_review_v2_probe.py` v4 固定。规则和最终证据见 `docs/reference/playback-stats-rules.md` 与 `docs/reports/2026-08-12-yearly-review-v2-delivery.md`。
