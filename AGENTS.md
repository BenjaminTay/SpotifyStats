...
**AI Visual Yearly Report Artifact（2026-07-04/05）**：年度报告默认 `report_mode=visual_yearly_artifact` + `writer_pipeline=editorial_agent_v1`，保留只读 Report Agent 与 fact validator 作为证据和事实安全网。后端先生成 Research Brief、Storyline Plan 与 deterministic chart data，再由 Editorial Agent 完成长文写作、编辑、Claim Checker 与 Taste Rubric；artifact 组装层负责事实去重、必要实体补齐、图表观察解释、partial-year 文案修正和坏词缝清洗。LLM 只负责故事表达和章节文案，不得生成图表数据；图表数据必须由 deterministic backend builder 从只读播放与个人榜单数据生成。缓存 key 必须包含 `visual_yearly_artifact`、`visual_yearly_v1` 与 writer pipeline，避免旧 Markdown/agentic/deterministic 年报缓存挡住新结果；前端必须优先渲染 `artifact`，无 artifact 时才回退 Markdown。新增年报能力时优先扩展 `backend/domains/ai_reports/editorial_agent/`、artifact builders、critic/probe 和验收矩阵，不要把更多一次性 DATA 拼进 prompt。
