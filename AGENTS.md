...
**AI Visual Yearly Report Artifact（2026-07-04）**：年度报告默认 `report_mode=visual_yearly_artifact`，保留只读 Report Agent 与 fact validator 作为证据和事实安全网，新增 Narrative Brief、Visual Brief、Chart Data Builder、Visual Critic 与前端 artifact renderer。LLM 只负责故事表达和章节文案，不得生成图表数据；图表数据必须由 deterministic backend builder 从只读播放与个人榜单数据生成。缓存 key 必须包含 `visual_yearly_artifact` 与 `visual_yearly_v1`，避免旧 Markdown/agentic 年报缓存挡住图文 artifact；前端必须优先渲染 `artifact`，无 artifact 时才回退 Markdown。
