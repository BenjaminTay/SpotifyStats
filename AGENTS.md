...
`.dockerignore` 必须持续排除整个 `data/`、备份和环境密钥，SQLite、封面及原始导出只通过 `/opt/spotify-stats/data/` 持久化迁移。生产发布使用 commit SHA 镜像、发布前 Online Backup、三模式门禁、健康检查，并将镜像 SHA、部署模式和发布前 SQLite 作为一个联合回滚单元；每日服务器内备份不能替代异机备份。GitHub 构建的 `linux/amd64` 镜像经短期私有 CAS Artifact 只把缺失 blob 续传到现有服务器，服务器校验 image/revision/digest 后推送 TCR；所有镜像传输完成前不得备份、预检或停服。音乐搜索预检只在明确 DB 副本执行 migration 36、候选版本和精确六变体校验：候选索引与统计 fingerprint 独立，普通 SHA、前端、部署脚本或查询匹配变化不得冷建六套统计；旧库首次统计引导必须使用不部署应用的独立 workflow。停服后源数据发生漂移即拒绝替换，禁止在 live DB 首次冷构建。3000/3001/3002/8000 均不得直接开放公网。`SPOTIFY_STATS_REQUIRE_AUTH` 只保护部分写接口，不能被描述为整站认证；完全版外部入口必须另有身份边界，公开入口则是持链接者均可访问。OAuth 回调始终精确使用完全版的 HTTPS 地址。完整规则见 `docs/plans/2026-08-13-dual-deployment-profile-plan.md` 与 `docs/reports/2026-08-13-dual-deployment-profile-delivery.md`。
...
**年度总结 V2（2026-08-12）**：`/yearly-review` 是单一的自有年度总结入口；Desktop/Compact 与 Phone 共享确定性八章 `YearlyReviewV2` 数据、过滤指纹、缓存和后台生成链路，但保持互斥、独立的 presentation。桌面使用完整杂志年鉴，Phone 使用“口袋音乐年鉴”、2×3 KPI、紧凑章节进度 Sheet、纵向时间线和无宽表的全屏榜单。官方 Wrapped 页签及前端组件已退役；`/api/wrapped-hub`、官方导入表与读取服务仅作只读兼容冻结，不再新增消费 UI 或产品能力。后端只读 API 为 `/api/yearly-review/available-years`、`/{year}`、`/{year}/records`；主报告与兼容 records 响应必须共享完整过滤指纹、策略版本、display taxonomy/艺人元数据 revision 和缓存语义。月度正文只保留一条转折时间线，十二月明细进入可展开账本；播放榜与个人 Billboard 不得混名，YTD/partial/unknown 必须由 coverage 控制措辞，LLM 不参与事实生成。同比只使用真实 aligned window，工作日/周末使用观察区间自然日日均，Passport 与播放榜共享规范实体粒度；YTD 品味只比较完整季度，不足时只展示分布。公开纪录必须命中显式 renderer 并具备周期/数值证据，阶段无法证明时为空，结语不得复制开篇。消费界面只使用普通用户能理解的文案，不展示口径、指纹、策略、证据等级、coverage 或 limitations；六项年度 KPI 只用红/绿箭头与百分比表示同比，实体故事和完整榜单优先显示封面及详情深链，大标题不附加解释性 subtitle。年份按钮按升序排列、只显示年份，并默认最新可用年份（即使当前年度尚未结束）；完整年度封面不显示状态/日期，进行中报告仍在封面保留截止日期；封面不再显示三条头条或海报操作。所有年度动态文案与实体名称必须遵循全局简繁体偏好；回归、发现等时间线事件必须明确对象是歌曲、专辑还是艺人，避免与详情页的歌曲/专辑榜单混淆。新关系按实体分别称为“今年发现的新歌 / 今年新听的专辑 / 今年认识的新艺人”，同专辑或艺人多首入榜纪录必须写明准确曲目数；宽屏分歧故事两列、窄屏单列。年度纪录章只展示 6–8 条精选，不提供“更多年度纪录”目录；兼容 `/records` 只返回与正文相同的精选集合，artifact 不再序列化上千条候选。桌面章节纵向 padding 统一为 `clamp(48px, 5.5vw, 80px)`；Phone 主要触控目标至少 44×44px，完整榜单正文只预览 Top 5，全屏每页 10 条并在关闭后恢复焦点。schema version 与 content version 分开治理，任何统计/编排/公开展示语义变化都必须提升 content version。当前 content 为 v2.13；v2.8 的 2023–2026 性能基线为真实重算 10.65–16.54s、热响应 26.56–29.85ms、跨进程持久命中 10.20–21.07ms。公开文案、封面/深链、YTD 措辞、精选证据、结语去重和阶段状态由 `scripts/yearly_review_v2_probe.py` v5 固定。规则和最终证据见 `docs/reference/playback-stats-rules.md` 与 `docs/reports/2026-08-12-yearly-review-v2-delivery.md`。


**逻辑播放时间线 V2（2026-08-15）**：连续同曲同来源默认只在实际空闲时间不超过 5 分钟时合并；实际空闲按下一行推断开始时间减上一行停止时间计算，不得使用两个停止时间之差。自然日、月份、年份与 Billboard 周不能作为合并边界。完整事件在累计收听达到歌曲时长时计数，余数事件在达到有效阈值时计数；每个事件使用独立 `counted_at`，播放次数按 `counted_at` 归属，收听时长按 `[ts-ms_played, ts)` 推断区间切到北京时间小时、自然日与榜单周。Billboard 原始与预聚合路径必须使用独立 `play_count`、`total_ms` 权重；预聚合通过临时影子表和单事务发布，失败保留旧快照。详情页摘要、分布和时长排行必须从实体范围重新生成时长切片，不能继承全库 `DataFrame.attrs`；艺人最近 50 次按 `_logical_event_id` 去重，歌曲版本组/专辑发行组复用同一加权帧。播放分析与详情的 `last_4_weeks` / `last_6_months` 以最新有效数据日锚定。服务端 Settings 的 `max_merge_gap_minutes=5` 是唯一默认事实源，Desktop、Phone 不得恢复独立 localStorage 默认值。完整规则见 `docs/reference/playback-stats-rules.md` 与 `docs/reports/2026-08-17-music-detail-statistics-fix-delivery.md`。
...
**音乐查找 V2（2026-08-16）**：Masthead Quick Open 与 `/music/search` 只搜索本地歌曲、album project 和 canonical artist，并继续打开既有三类详情深链；不要新增顶级入口或在“播放排行”重复放搜索框。交互链路固定为 `response_mode=candidates` 先读版本化 FTS5 trigram 派生索引，再由 `/api/music/search/context` 按稳定 `entity_key` 读取同一完整过滤指纹的精确快照；候选 GET 禁止加载 lifetime 播放帧或计算完整 Billboard。候选索引使用确定性 `candidate_index_version`，六套统计使用不含随机 generation/Git SHA 的 `statistics_fingerprint`；只有对应 revision 或 builder 内容真实变化才重建相应层。查询支持固定 OpenCC 简繁双向扩展、单/双字 CJK n-gram，以及仅在主路径零命中且长度至少 4 时启用的最多 50 条 trigram 候选池和有界编辑距离。服务端基础设置必须维护 L1/L2/L3 × 动态阈值开/关六个 snapshot 变体，由一个 `snapshot-set` job 顺序构建；GET 只读持久 revision state，并且 snapshot reader 同时校验 `ready`、精确 fingerprint 与当前 builder version。候选与 context 必须分型，未加载统计不得展示为 0；前端只展示播放次数、`PK #`、在榜周数和走势排名，仍不展示冠军周数。Quick Open 每类 3 条、默认不选首条；全部页每类 5 条预览，单类型每页 20 条并保存 `q/kind/page`。输入统一使用 220ms 防抖、IME composition 门禁、AbortSignal、`retry: 0`、keep-previous-data 和语义 query key；仅 warming/stale 退避观察，终态、错误、页面隐藏和卸载停止。Unicode 高亮必须在 `Intl.Segmenter` 缺失或不可构造时安全降级。public 只允许 `eligibility=current` 且 GET 绝不写库、排队或冷构建；`any_local` 只供 private Settings。导入、统计设置、版本归并、艺人身份和曲目署名变化必须 bump 对应持久 revision；候选与统计分别按其依赖失效，禁止无差别联合重建。startup catch-up 使用独立 `SPOTIFY_STATS_SEARCH_STARTUP_REBUILD`，正常 private 生产默认开启，副本 one-shot 显式关闭；精确 ready 集合必须只校验不重复重建。性能门禁为候选 warm P95 ≤80ms、context P95 ≤20ms、响应 ≤8KiB；完整规则和证据见 `docs/plans/2026-08-16-music-search-direction-realignment.md` 与 `docs/reports/2026-08-16-music-search-optimization-delivery.md`。

**音乐查找展示规则（2026-08-17）**：`match_field` / `match_type` 只供排序、高亮、测试和诊断，消费界面不展示“匹配艺人/专辑/别名”“简繁匹配”“近似匹配”等工程标签。搜索结果名称、副标题、封面辅助文字与最近查看列表必须跟随全局简繁体偏好；转换仅发生在展示层，不得改变稳定键、详情深链、索引内容或保存的原始值。
...
`base_filters()` 是 SQL 粗过滤入口：`music_only` 排除播客；未合并路径直接应用 `ms_played >= min_ms`。合并连续播放（`merge_enabled`，默认开启）使用 `min_ms=0` 先保留短片段，按 `track_id` + `source_album_id` + 默认 5 分钟实际空闲边界重建逻辑事件，日期与榜单周仅在重建后归属，再由 `filter_effective_plays()` 应用固定阈值或动态阈值。已移除不可靠的 `skipped` 过滤。
