...
`.dockerignore` 必须持续排除整个 `data/`、备份和环境密钥，SQLite、封面及原始导出只通过 `/opt/spotify-stats/data/` 持久化迁移。生产发布使用 commit SHA 镜像、发布前 Online Backup、三模式门禁、健康检查，并将镜像 SHA、部署模式和发布前 SQLite 作为一个联合回滚单元；每日服务器内备份不能替代异机备份。涉及音乐搜索 schema/快照的新 SHA 必须先在明确 DB 副本完成容量门禁、migration 34、精确六变体与 orphan=0 one-shot，停服后源数据发生漂移即拒绝替换，禁止在 live DB 首次冷构建。3000/3001/3002/8000 均不得直接开放公网。`SPOTIFY_STATS_REQUIRE_AUTH` 只保护部分写接口，不能被描述为整站认证；完全版外部入口必须另有身份边界，公开入口则是持链接者均可访问。OAuth 回调始终精确使用完全版的 HTTPS 地址。完整规则见 `docs/plans/2026-08-13-dual-deployment-profile-plan.md` 与 `docs/reports/2026-08-13-dual-deployment-profile-delivery.md`。
...
**音乐查找 V2（2026-08-16）**：Masthead Quick Open 与 `/music/search` 只搜索本地歌曲、album project 和 canonical artist，并继续打开既有三类详情深链；不要新增顶级入口或在“播放排行”重复放搜索框。交互链路固定为 `response_mode=candidates` 先读版本化 FTS5 trigram 派生索引，再由 `/api/music/search/context` 按稳定 `entity_key` 读取同一完整过滤指纹的精确快照；候选 GET 禁止加载 lifetime 播放帧或计算完整 Billboard。服务端基础设置必须维护 L1/L2/L3 × 动态阈值开/关六个 snapshot 变体，由一个 `snapshot-set` job 顺序构建；GET 只读持久 revision state，并且 snapshot reader 同时校验 `ready`、精确 fingerprint 与当前 builder version。候选与 context 必须分型，未加载统计不得展示为 0；前端只展示播放次数、`PK #`、在榜周数和走势排名，仍不展示冠军周数。Quick Open 每类 3 条、默认不选首条；全部页每类 5 条预览，单类型每页 20 条并保存 `q/kind/page`。输入统一使用 220ms 防抖、IME composition 门禁、AbortSignal、`retry: 0`、keep-previous-data 和语义 query key；仅 warming/stale 退避观察，终态、错误、页面隐藏和卸载停止。Unicode 高亮必须在 `Intl.Segmenter` 缺失或不可构造时安全降级。public 只允许 `eligibility=current` 且 GET 绝不写库、排队或冷构建；`any_local` 只供 private Settings。导入、统计设置、版本归并、艺人身份和曲目署名变化必须 bump 对应持久 revision，失效并重建整套六变体。startup catch-up 使用独立 `SPOTIFY_STATS_SEARCH_STARTUP_REBUILD`，正常 private 生产默认开启，副本 one-shot 显式关闭；精确 ready 集合必须只校验不重复重建。性能门禁为候选 warm P95 ≤80ms、context P95 ≤20ms、响应 ≤8KiB；完整规则和证据见 `docs/plans/2026-08-16-music-search-performance-and-experience-optimization-plan.md` 与 `docs/reports/2026-08-16-music-search-optimization-delivery.md`。
