...
| `core/migrations.py` | 版本化 Migration，`IF NOT EXISTS` 幂等，`schema_migrations` 表追踪；AI task run/event/tool-call 表在迁移中创建 |
...
| 文件 | 职责 |
|------|------|
| `services/play_service.py` | 核心播放数据服务，所有基于 plays 的端点统一入口；负责播放过滤参数贯穿，包含深夜歌曲排行等时段分析只读聚合 |
...
| `services/ai_insights_service.py` | AI 洞察：周报/月报/年度叙事 + 自然语言问答 + 推荐问题随机池（年度叙事默认 `visual_yearly_artifact` + `writer_pipeline=agent_synthesis_v2`，复用报告数据契约、质量 validator、cache contract version 与确定性 fallback；复用 LLM 基建 + wikipedia_cache 表） |
| `services/ai_task_service.py` | AI task 编排：报告 cache check/manual generation、图文年度报告 artifact、艺人/专辑 enrichment、任务状态与事件持久化 |
...
- `domains/account_archive/` — 音乐档案白名单事实：overview、收藏旅程、固定窗关系、回归、隐私安全发现统计、服务端收藏库与其他媒体；旧重型 account summary、人格与 chemistry 不得恢复。
...
- `domains/ai_reports/` — AI 年度报告工具、契约、validator、agentic longform、visual yearly artifact 与 Report Agent 写作流水线；图表数据必须由只读 builder 生成，不能由 LLM 编造
- `domains/ai_agent/` — 后端定义的只读 Agent 工具注册表、question intent、QuestionFrame、EvidenceRecipe、EvidenceSufficiency、AnalyticalBrief、temporal context/guard、temporal guard 到 EvidenceRecipe/AnalyticalBrief 的 custom range 投影、entity resolver、compare_entities、coverage review、answer critic、golden harness、evidence builders 与 handler；当前覆盖总体统计、排行、播放记录、年度总结、实体统计、个人 Billboard 实体详情、听歌时段/深夜歌曲排行、账号收藏、搜索历史、社区帖子搜索和社区热议趋势
...
- AI 年度叙事默认由 `backend/domains/ai_reports/visual_yearly_artifact_service.py` 生成 `visual_yearly_artifact`，并通过 `backend/domains/ai_reports/report_agent.py` 的 Report Agent 多轮工具调用（含 web_search）直接输出报告 JSON：`chart_specs`、`chart_data`、个人榜单关系和事实校验必须来自只读后端 builder/validator，LLM 不得生成图表数据；artifact 组装层要继续负责事实去重、必要实体补齐、图表观察解释、partial-year 文案修正和坏词缝清洗。`backend/services/yearly_report_agent_service.py` / `agentic_tools.py` 保留为 `agentic_longform` 兼容模式和只读证据基础。`backend/domains/ai_reports/yearly_contract.py` 与 `yearly_validator.py` 仍维护事实安全口径；2026 这类未完整年份必须按 `reporting_period.end_date` 写成阶段性/年中总结，只能用 same-period YTD 对比，不得混用完整上一年跌幅、编造场景或隐藏 TOP 名称；critic、claim checker、taste rubric 或 validator 失败时只能标记明确 fallback。
