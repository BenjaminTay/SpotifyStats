...
| `services/ai_agent_service.py` | 只读 AI Agent 问答：LLM 规划工具、执行 allowlist 工具、压缩 evidence、coverage 自检与必要时重试回答 |
...
- `domains/ai_agent/` — 后端定义的只读 Agent 工具注册表与 handler，当前覆盖总体统计、排行、播放记录、年度总结、实体统计、个人 Billboard 实体详情和听歌时段
