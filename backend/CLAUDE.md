...
| `services/ai_agent_service.py` | 只读 AI Agent 问答：LLM 规划工具、执行 allowlist 工具、压缩 evidence cards、coverage 自检、answer critic 与必要时重试回答 |
...
- `domains/ai_agent/` — 后端定义的只读 Agent 工具注册表、question intent、entity resolver、compare_entities、coverage review、answer critic、evidence builders 与 handler；当前覆盖总体统计、排行、播放记录、年度总结、实体统计、个人 Billboard 实体详情和听歌时段/深夜歌曲排行
...
- AI Agent 工具结果必须保留 `params_summary`、`result_summary`、`source_range` 和必要 compact evidence；最终回答 prompt 必须遵守 coverage，不得把 found 实体或个人 Billboard 结果说成缺失
