...
三层 pytest markers：`unit`（纯函数，无 DB，~5s）→ `contract`（seed DB 结构验证，~1s）→ `integration`（真实数据只读，~80s）。当前基线：unit 234 / contract 127 / backend full 561。补充只读 API smoke 使用 `.venv/bin/python scripts/api_smoke_probe.py`，覆盖 91 个本地 GET、验证 `X-Request-ID`，并核算 OpenAPI GET 覆盖（未核算路径必须为 0）。
