...
| `services/ai_insights_service.py` | AI 洞察：周报/月报/年度叙事 + 自然语言问答 + 推荐问题随机池（年度叙事默认 `visual_yearly_artifact` + `writer_pipeline=editorial_agent_v1`，复用报告数据契约、质量 validator、cache contract version、Editorial Agent 与确定性 fallback；复用 LLM 基建 + wikipedia_cache 表） |
...
- `domains/ai_reports/` — AI 年度报告工具、契约、validator、agentic longform、visual yearly artifact 与 `editorial_agent/` 写作流水线；图表数据必须由只读 builder 生成，不能由 LLM 编造
