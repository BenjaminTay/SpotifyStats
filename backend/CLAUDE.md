...
三层 pytest markers：`unit`（纯函数，无 DB，~5s）→ `contract`（seed DB 结构验证，~1s）→ `integration`（真实数据只读，~80s）。当前基线：unit 233 / contract 127 / backend full 560。补充只读 API smoke 使用 `.venv/bin/python scripts/api_smoke_probe.py`，覆盖 91 个本地 GET 并验证 `X-Request-ID`。
