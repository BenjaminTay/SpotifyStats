...
| `services/ai_insights_service.py` | AI 洞察：周报/月报/年度叙事 + 自然语言问答 + 推荐问题随机池（年度叙事默认 `visual_yearly_artifact`，复用报告数据契约、质量 validator、cache contract version 与确定性 fallback；复用 LLM 基建 + wikipedia_cache 表） |
...
- `domains/ai_reports/` — AI 年度报告工具、契约、validator、agentic longform 与 visual yearly artifact；图表数据必须由只读 builder 生成，不能由 LLM 编造
...
- AI 年度叙事默认由 `backend/domains/ai_reports/visual_yearly_artifact_service.py` 生成 `visual_yearly_artifact`：Narrative Brief 和章节文案可由规则/LLM 辅助，但 `chart_specs`、`chart_data`、个人榜单关系和事实校验必须来自只读后端 builder/validator；`backend/services/yearly_report_agent_service.py` / `agentic_tools.py` 保留为 `agentic_longform` 兼容模式和只读证据基础。`backend/domains/ai_reports/yearly_contract.py` 与 `yearly_validator.py` 仍维护事实安全口径；2026 这类未完整年份必须按 `reporting_period.end_date` 写成阶段性/年中总结，只能用 same-period YTD 对比，不得混用完整上一年跌幅、编造场景或隐藏 TOP 名称；critic 或 validator 失败时只能标记明确 fallback。
