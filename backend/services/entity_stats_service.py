...
from backend.services.play_service import _album_cover_lookup, _artist_cover_lookup, _track_cover_urls
...
def _rank_for(rows: list[dict], entity: str, *, track_id=None, album_name=None, artist_name=None) -> Optional[int]:
    for row in rows:
        if entity == "track" and row.get("track_id") == track_id:
            return row["rank"]
        if entity == "album" and row.get("album_name") == album_name and row.get("artist_name") == artist_name:
            return row["rank"]
...
def _ranks(
    ...
    for key, resolved in periods.items():
        ...
        result[key] = _rank_for(rows, entity, track_id=track_id, album_name=album_name, artist_name=artist_name)
    _, rows = chart_rows(conn, current_df, entity, "plays", None, 0)
    result["current_period"] = _rank_for(rows, entity, track_id=track_id, album_name=album_name, artist_name=artist_name)
    return result
...
def _top250_count(
    ...
    *,
    ...
    count = 0
    for row in rows:
        if album_name is not None and row.get("album_name") == album_name and row.get("artist_name") == artist_name:
            count += 1
...
def _top250_counts(conn: sqlite3.Connection, all_df: pd.DataFrame, *, album_name=None, artist_name=None) -> dict:
    return {
...
def get_track_stats(
    ...
    all_df, current_df, resolved = load_period_plays(conn, min_ms, music_only, merge_enabled, period, start_date, end_date)
...
def get_album_stats(
    ...
    entity_all = all_df[(all_df["album_name"] == album_name) & (all_df["artist_name"] == artist_name)]
    entity_df = current_df[(current_df["album_name"] == album_name) & (current_df["artist_name"] == artist_name)]
    if entity_all.empty:
        return {"found": False}
    data = _entity_base(all_df, entity_df)
    ...
    data.update({
        ...
        "first_played": str(entity_all["ts"].min()),
        "last_played": str(entity_all["ts"].max()),
        "ranks": _ranks(conn, all_df, current_df, "album", album_name=album_name, artist_name=artist_name),
        "top250_counts": _top250_counts(conn, all_df, album_name=album_name, artist_name=artist_name),
