...
alone is not an album identity.  This module only proposes a release merge when
every participating standalone Album Project independently proves the same
complete Spotify release through matching release metadata and at least one
local Spotify track contained in that release's complete track list.
...
ALBUM_PROJECT_AUTO_IDENTITY_POLICY_VERSION = "spotify_complete_release_v1"
...
def normalize_album_release_name(value: str | None) -> str:
    ...
    # album project. This only broadens metadata matching; a merge still needs
    # the exact same Spotify album ID and complete track-list proof below.
...
def plan_album_project_auto_merges(conn: sqlite3.Connection) -> AlbumProjectAutoMergePlan:
    ...
    * at least one locally linked Spotify track must occur in the complete
      Spotify track list;
    * every project in a candidate must point unambiguously at the same strong
      release evidence.

    This deliberately leaves existing deluxe/bundle projects such as Showgirl
    and folklore/Long Pond untouched, and freezes compilation policy.
    """
    ...
    evidence_groups: dict[tuple[str, str, str, str, str], list[_ProjectReleaseEvidence]] = (
        defaultdict(list)
    )
    for project_evidence in evidence_by_project.values():
        for evidence_row in project_evidence:
            evidence_groups[_release_fingerprint(evidence_row)].append(evidence_row)

    project_candidate_keys: dict[int, set[tuple[str, str, str, str, str]]] = defaultdict(set)
    for fingerprint, grouped_evidence in evidence_groups.items():
        if len({item.project_id for item in grouped_evidence}) < 2:
            continue
        for item in grouped_evidence:
            project_candidate_keys[item.project_id].add(fingerprint)
    ambiguous_projects = {
        project_id
        for project_id, fingerprints in project_candidate_keys.items()
        if len(fingerprints) > 1
    }
    skipped["ambiguous_complete_release_evidence"] += len(ambiguous_projects)

    candidates: list[AlbumProjectAutoMergeCandidate] = []
    for fingerprint, evidence_rows in sorted(evidence_groups.items()):
        by_project = {item.project_id: item for item in evidence_rows}
        if len(by_project) < 2 or any(pid in ambiguous_projects for pid in by_project):
            continue
        candidate_evidence = sorted(by_project.values(), key=lambda item: item.project_id)
        spotify_album_id, _normalized_name, release_date, _artists_key, _track_hash = fingerprint
        album_ids = tuple(sorted(item.album_id for item in candidate_evidence))
        project_ids = tuple(sorted(item.project_id for item in candidate_evidence))
        primary = _select_primary_evidence(candidate_evidence)
        artist_id = _select_project_artist_id(conn, candidate_evidence, primary)
        candidate_key = _candidate_key(spotify_album_id, album_ids)
        candidates.append(
        ...
        for candidate in plan.candidates:
            ...
            conn.execute(
                """INSERT INTO album_project_external_ids(
                   ...
                   ) VALUES ('spotify', ?, ?, 'catalog_exact', 1.0, 1)
                ...
                (candidate.spotify_album_id, project_id),
            )
...
def _strong_release_evidence_for_album(
    ...
    *,
    ...
    for row in rows:
        ...
        if normalize_album_release_name(project_name) != normalize_album_release_name(spotify_name):
...
def _candidate_key(spotify_album_id: str, album_ids: tuple[int, ...]) -> str:
    payload = f"spotify:{spotify_album_id}|albums:{','.join(map(str, album_ids))}"
...
def _validate_plan_is_fresh(
    ...
    current_by_key = {candidate.candidate_key: candidate for candidate in current.candidates}
    for candidate in plan.candidates:
        if current_by_key.get(candidate.candidate_key) != candidate:
            raise RuntimeError(f"stale Album Project auto-merge plan: {candidate.candidate_key}")
