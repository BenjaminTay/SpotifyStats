"""Compressed Governance family results with atomic active/previous publication."""
...
def path():
    return Path(
        ...
        or Path(db.DB_PATH).with_name("governance_cache.db")
    )
...
def connect(*, write=False):
    ...
    if write:
        if public_readonly_db_guard_active():
            raise PermissionError("Public requests cannot write Governance snapshots")
        ...
        with conn:
            ...
            if (
                ...
                * conn.execute("PRAGMA page_size").fetchone()[0]
                ...
                raise ValueError("Governance active/previous storage exceeds 30 MiB")
            fence()
    finally:
        conn.close()
