    ...
    *,
    ...
    return {
        ...
        "metric_definitions": {
            ...
            "个人历史播放里程碑": (
                "在完整有效播放历史中计算累计序号，只展示阈值事件发生在报告年度内的节点。"
            ),
            "年度新发现": (
                "按规范实体的个人完整历史首次出现日期计算，不把年度内再次出现视为新发现。"
            ),
            "含艺人的播放占比": (
                "包含该署名艺人的去重逻辑播放事件数除以年度全部逻辑播放事件数；"
                "合作歌曲可同时计入多位艺人。"
            ),
            "未完整月份比较": ("本月至观察截止日与上月同期或上年同期的等长日期范围比较。"),
        },
        "comparison_periods": {
        ...
        "notes": [
            "所有章节共享同一套有效播放口径。",
            "所有歌曲数量、复听和发现事实共享当前曲目归并级别下的规范曲目身份。",
            "唯一最高级只用于对应指标的第一名；并列第一会明确标注。",
