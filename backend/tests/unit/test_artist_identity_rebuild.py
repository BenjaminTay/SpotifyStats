...
def _database(path) -> None:
    ...
    conn.executescript(
        ...
        INSERT INTO artist_identity_state VALUES (1, 4, 3, 'pending', NULL, NULL);
...
def test_shadow_rebuild_atomically_switches_artist_aggregates(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: pd.DataFrame(
            [
                {"billboard_week": "2026-02-06", "artist_id": 532, "ms_played": 1000},
                {"billboard_week": "2026-02-06", "artist_id": 532, "ms_played": 2000},
            ]
        ),
    )

    rebuild.handle_artist_identity_rebuild(
        Job.create("artist_identity_rebuild", "artist_identity", "global", revision=4)
    )

    conn = _connection_factory(path)()
    assert [tuple(row) for row in conn.execute("SELECT * FROM agg_weekly_artists")] == [
        ("2026-02-06", 532, 2, 3000)
    ]
...
def test_failed_rebuild_keeps_active_aggregate_and_marks_realtime_fallback(tmp_path, monkeypatch):
    path = tmp_path / "identity-failure.db"
    _database(path)
    monkeypatch.setattr(rebuild, "get_db", _connection_factory(path))
    monkeypatch.setattr(rebuild, "SettingsRepository", _Settings)
