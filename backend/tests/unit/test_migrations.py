...
def test_migrate_073_repairs_agent_branch_version_69_collision(empty_db):
    """An existing Agent migration 69 must not suppress L3 coverage tables."""
    from backend.core import migrations

    migrations.migrate_068(empty_db)
    _ensure_migrations_table(empty_db)
    empty_db.execute(
        "INSERT INTO schema_migrations(version, name) VALUES (69, ?)",
        ("ai_agent_turn_event_log",),
    )

    migrations.migrate_073(empty_db)

    columns = {
        str(row[1])
        for row in empty_db.execute("PRAGMA table_info(l3_album_attribution_revision_state)")
    }
    assert {"scanned_count", "excluded_count"} <= columns
    table = empty_db.execute(
        "SELECT name FROM sqlite_master "
        "WHERE type='table' AND name='l3_song_album_attribution_exclusions'"
    ).fetchone()
    assert table is not None
