...
- **年度总结** — 播放分析内的自定义 Wrapped 总结（听歌人格识别 6 型、曲风五大洲全景、基于已审核艺人事实的动态语言分布、发现与回归、聆听深度金字塔、特殊时刻、年度对比）+ 官方 Wrapped 数据；语言与流派独立统计，并保留未知、多语言、纯器乐及未归属时长说明
...
- **设置** — Spotify OAuth 连接管理、LLM 翻译配置（多提供商 + 档案管理）、数据过滤、版本合并、数据导入；卡片 06“流派与语言数据健康”提供语言覆盖率、Top unknown、艺人搜索、状态历史和证据审核，候选数据需人工批准后才进入统计
