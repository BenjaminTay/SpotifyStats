...
- **年度总结** — 播放分析内的自定义 Wrapped 总结（听歌人格识别 6 型、四轴 genre 全景、基于已审核艺人事实的动态语言分布、发现与回归、聆听深度金字塔、特殊时刻、年度对比）+ 官方 Wrapped 数据；genre 的 style / scene / context / role 独立计算，主榜只展示 style，语言与流派独立统计，并保留未知、多语言、纯器乐及未归属时长说明
- **Billboard 周榜 / 年榜** — 周榜、每周榜首、年度单曲/专辑/艺人榜、走势总榜 Power Score、总榜、榜单记录、对决、发行周期分析；对决与实体详情共享同一统计设置和专辑项目/艺人身份口径
...
- **设置** — Spotify OAuth 连接管理、LLM 翻译配置（多提供商 + 档案管理）、数据过滤、版本合并、数据导入；卡片 06“流派与语言数据健康”提供各轴覆盖率、Top unknown、艺人搜索、来源置信度、风险提示、状态历史和证据审核，genre 审核必须先保存 HTTPS 证据，候选数据需人工批准后才进入统计
