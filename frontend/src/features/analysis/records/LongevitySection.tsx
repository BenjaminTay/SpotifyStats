    ...
    <div>
      ...
      <EntityRecordCard title="最长陪伴跨度 · Longest Span" subtitle="首次播放到最近一次播放日期跨度最长的歌曲/专辑/艺人"
        ...
        columns={(entity) => [
          { header: '#', width: '48px', align: 'center', render: (_, i) => <RankNum rank={i + 1} /> },
          entityNameCol(entity),
          { header: '跨度', width: '140px', align: 'right', render: (row) => <span className="font-serif text-[20px] font-semibold tabular-nums">{row.value}<span className="ml-1 font-sans text-[12px] font-normal text-muted-foreground">{displayName(row.unit)}</span></span> },
      ...
      <EntityRecordCard title="沉睡后回归 · Comeback After Sleep" subtitle="同一实体两次出现之间间隔最长，并在后一次重新出现"
        ...
        columns={(entity) => [
          { header: '#', width: '48px', align: 'center', render: (_, i) => <RankNum rank={i + 1} /> },
          entityNameCol(entity),
          { header: '间隔', width: '140px', align: 'right', render: (row) => <span className="font-serif text-[20px] font-semibold tabular-nums">{row.value}<span className="ml-1 font-sans text-[12px] font-normal text-muted-foreground">{displayName(row.unit)}</span></span> },
          { header: '沉睡→唤醒', width: '200px', align: 'right', render: (row) => <span className="font-sans text-[12px] text-muted-foreground">{row.start_date ?? '?'} → {row.end_date ?? '?'}</span> },
        ]} />
