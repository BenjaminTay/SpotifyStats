...
export function DiscoverySection({ data }: Props) {
  return (
    <div>
      ...
      <EntityRecordCard title="合作曲偏好 · Feat Lover" subtitle="合作歌曲（含 feat/with/& 标记）播放佔比与排行"
        recordsByEntity={{ track: data.feat_lover?.track ?? [], artist: data.feat_lover?.artist ?? [] }} defaultEntity="track"
        columns={(entity) => [
          { header: '#', width: '48px', align: 'center', render: (_, i) => <RankNum rank={i + 1} /> },
          entity === 'track'
