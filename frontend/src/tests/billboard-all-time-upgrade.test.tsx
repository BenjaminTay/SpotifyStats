...
describe('Billboard all-time search and column schema', () => {
  ...
  it('keeps fixed power ranks unchanged under arbitrary sorting and renders numeric columns right-aligned', () => {
    ...
    const chartPlaysGroup = chartPlaysCell?.firstElementChild
    const chartPlaysBar = chartPlaysCell?.querySelector('[role="meter"]')
    expect(chartPlaysGroup).toHaveClass('inline-flex', 'items-center')
    expect(chartPlaysGroup?.firstElementChild).toHaveTextContent('100')
    expect(chartPlaysGroup?.lastElementChild).toBe(chartPlaysBar)
    expect(chartPlaysBar).toHaveAttribute('aria-valuenow', '100')
    expect(chartPlaysBar).toHaveClass('w-[56px]')
