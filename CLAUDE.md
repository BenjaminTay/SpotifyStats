...
- **AI 年度叙事 → `visual_yearly_artifact_service.py` + `report_writer.py` + `visual_chart_data.py` + `visual_yearly_critic.py`**；默认年度报告必须返回 `visual_yearly_artifact` 且走 `writer_pipeline=agent_synthesis_v2`，图表数据只能由 deterministic backend builder 生成；writer metadata、critic 与 fact validation 都要进入 task result/cache metadata；`yearly_report_agent_service.py` / `agentic_tools.py` 保留为只读证据与 `agentic_longform` 兼容模式，`editorial_agent_v1` 映射到新路径，旧 `yearly_contract.py` / `yearly_validator.py` / 确定性 fallback 只作为事实安全网和 `basic_summary` 回退
