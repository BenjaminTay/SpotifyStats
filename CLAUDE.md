...
年度总结 V2：`/yearly-review` 只在 Desktop/Compact 挂载确定性八章个人音乐年鉴，Phone 保留 V1，官方 Wrapped 保持独立。V2 通过 `/api/yearly-review/available-years|{year}|{year}/records` 消费统一过滤指纹、coverage、策略与元数据 revision；播放/时长榜和个人 Billboard 必须明确区分，月度正文只有一条转折时间线，十二月明细和完整纪录按需展开/分页。同比使用真实 aligned window，工作日/周末使用自然日日均，Passport 使用规范实体粒度，YTD 品味只比较完整季度；公开纪录、阶段和结语均受显式证据规则约束。LLM 不生成年度事实。schema/content version 分开治理，统计或编排语义变化必须提升 content version。主 JSON 预算 512 KiB、热响应 250ms、真实重算预算 30s；关系历史禁止逐实体反复扫描全历史。最终 artifact 使用独立 sidecar SQLite 做精确键持久缓存，播放事实 revision 不依赖 SQLite main/WAL 物理 mtime，不返回 stale 数据；启动、设置变更和流式导入会后台预建最新年份。content v2.6 四年真实重算 9.80–15.43s、热响应 5.04–21.65ms、跨进程持久命中 9.75–19.71ms，probe v4 同时锁定公开文案、精选证据、结语去重与阶段状态，详见 `docs/reports/2026-08-12-yearly-review-v2-delivery.md`。
