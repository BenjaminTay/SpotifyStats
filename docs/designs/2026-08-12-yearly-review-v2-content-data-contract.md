...
- 新的年度总结 V2 后端契约与编排层。
- 新的桌面年度总结内容架构。
- 播放排行、个人 Billboard 年榜、年度纪录、时间线、关系故事、收听生活、品味迁移和完整索引。
...
`relationship_policy_v1` 支持的关系类型：
...
M0 已基于 2023–2025 三个完整自然年冻结 `relationship_policy_v1`：
...
正文默认精选 8–12 条，且每个一级类别最多 2 条。完整纪录目录由独立附录或原页面承接。
...
5. 最终选择：固定规则选出 8–12 条，不由 LLM 决定。

M0 冻结 `highlight_policy_v1`：
...
- 最终选择 8–12 条；每个一级类别最多 2 条、同一实体最多 2 条、同一底层指标最多 1 条。
- 在存在合格候选时，正文至少覆盖高峰/着迷、持续/陪伴、发现/回归、收听行为四种事实家族；个人 Billboard honors 已在第二章展示，不为凑数重复进入纪录簿。
- 若严格多样性约束后不足 8 条，可放宽类别上限到 3 条，但不得放宽证据、覆盖和语义去重门槛。
...
- 已知覆盖 40%–69.99%：允许显示含 unknown 的辅助图表，但只能给带限制说明的次要观察，不生成年度身份或主线结论。
- 已知覆盖 <40%：正文只提示覆盖不足；完整 buckets 和 unknown 比例放入附录/方法说明，不生成迁移结论。
...
- 月度冠军表。
- 年度纪录目录或前往播放记录/个人 Billboard records 的年度过滤深链。
- 数据范围、过滤口径、版本与方法说明。
...
完整纪录若使主响应明显膨胀，使用分页 `/records` endpoint；主响应仍必须返回精选纪录和目录数量。
...
1. V2 骨架：报告护照、双榜荣誉、唯一时间线、纪录精选、完整索引。
