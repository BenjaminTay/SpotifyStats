...
- 不完整月份的环比和同比只能分别与上月同期、上年同期的等长窗口比较，并在 metric 中同时记录观察与比较起止日期；完整月才可使用完整自然月比较。年度同比先尝试完整 aligned window；上一年覆盖不足时，取映射后的最大共同日历区间，至少 90 天才降级为 `common_period`，并把当前侧和基线侧窗口同时写入 coverage 与 metric。
...
- 无可比共同区间：隐藏同比，不显示 `0%` 或默认 `100%`；使用共同区间时，消费端明确显示“比去年同期”及两侧日期。
