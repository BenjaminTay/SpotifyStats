...
- Home、Billboard、播放分析的 LKG 状态改为仅供屏幕阅读器感知；音乐搜索在候选仍可用时隐藏索引维护横幅、“上一版本”徽标和更新行。
- 完全没有可用数据、真实请求错误、空结果、统计口径警告和设置页治理状态继续明确显示；普通页面不再透出“尚未发布、构建失败、重建任务”等维护详情。未改变请求、缓存、快照、统计或后台维护逻辑。

## 2026-09-21：阶段 8B Billboard 交互连续性

- 六个 Billboard 子页面改用持久路由外壳，桌面导航不再随页面 chunk 反复卸载；hover/focus 预取目标页面代码。
- Weekly 切周和切实体保留上一帧，并以响应自己的周/实体标签保证旧数据不冒充新选择；预取当前周相邻实体和相邻周，缓存返回不重复请求。
- All-Time 预取同视图的其他实体；页面级展示 LKG 更新状态。未改变 Billboard builder、快照 family、统计排序或响应字段。

## 2026-09-21：阶段 8A 快照状态与播放分析范围

- 全局快照提示改为消费页面内的用户语言，移除“等待发布”等后台管理术语；私有首页在 LKG 更新完成前持续轮询并自动切换。
- Analysis Stats / Records 自动维护 lifetime、近 4 周与近 6 个月；私有端缺失的年/月/周/日/自定义范围通过显式维护入口异步准备，公开 GET 继续零构建、零写入、零排队。
- 公开端时间选择器只展示自动维护的三个常用范围；快照 key、revision、source fence、LKG、统计 builder 与 L2/L3 语义保持不变。

## 2026-09-21：全应用性能优化成果总报告

- 汇总最初全应用性能审计、阶段 0～7E 的实施路径、关键代码/产品改变、前后性能指标、正确性边界和最终本地全栈验收。
- 明确总规划来自审计任务的《SpotifyStats 全应用性能审计与优化规划》，仓库内的阶段报告是执行证据；C1～C8 为候选设计，不等同于必须全部实现。
- 本次只新增成果报告与文档索引，不改变产品代码、统计合同或已完成的阶段 7E 验收结论。
...
- `d50f924c` 已进入 `origin/main` 并以 dual 模式部署；三容器 healthy、SQLite schema 73 / 92,908 条播放、搜索四变体、PWA 文件和服务器备份 timer 通过独立验收。Tailscale 外层入口当前停止，生产 LLM 凭据与 Spotify 连接未配置，未擅自改变这些 secret/external 状态。
...
- Billboard Records 同步收口板块导语和卡片说明；冠军传承单曲补充艺人名称及深链，箭头与新实体封面保持稳定留白，阻挡王的被阻挡歌曲标签增加小封面。
- 增加跨页面 UI 文案、冠军传承链接/间距和阻挡歌曲封面的回归测试；详细范围与验收边界见播放记录和 Billboard Records 交付报告。
...
- 修复歌曲详情仅统计当前 `track_id`、没有消费 L2/L3 分组的问题：摘要、最近播放和播放日历现在共用全局曲目榜的版本范围，并在范围内一次性重建逻辑事件。L2 不再误跟随 L3 作品父组，前端也将 URL 层级同步传给详情预取、播放列表和日历。
...
- 年榜历史视觉与周榜历史统一：移除独立覆盖范围列、完整年度标签和标题右侧重复说明，起始覆盖不足统一标为“不完整”；历史按年份从旧到新，Desktop 年份改用与年度积分一致的无衬线半粗字重，Phone 年份恢复 Playfair 半粗花体，排名使用两位 Playfair 数字与既有色阶。最高排名并列时以首次达到的年度为最佳且只在排名旁显示一次无边框纯文字 `PEAK`，排版与周榜变动 `RE / NEW` 共用同一组样式；文字不参与排名定位，Desktop 固定排名锚点及垂直中线、Phone 保留等宽且垂直居中的文字槽，并统一向下做 3px 光学校正。“年度上榜播放”增加同宽视觉条，Desktop 去掉单元格内重复单位并收紧年度/排名间距，榜单成绩 KPI 在有年榜摘要时使用 3×2 网格，“年榜入榜”只保留年度数量；Phone 重排为三层年度成绩卡，底部四项使用统一 24px 居中数值行，修复周榜峰值因字体行盒不同产生的下沉。
- 年榜历史的周成绩字段统一命名为“冠军周数 / 前五周数”；前五周数直接读取既有 `weeks_top5`，不再展示 `weeks_top10`。
- 年榜历史的年度数字新增类型感知深链：歌曲、专辑和艺人详情分别进入同年度单曲榜、专辑榜和艺人榜；年榜类型改为 `tab` URL 状态，支持直接落位、刷新和返回。
- 详情周榜历史的榜单周链接补齐实体类型：歌曲、专辑、艺人分别进入对应周次的单曲榜、专辑榜和艺人榜，Desktop/Phone 共用同一深链规则。单曲榜单成绩顶部统一复用专辑/艺人的卡片样式和主值/副文案层级；有年榜摘要时 Desktop 使用 3×2，首排依次为最高排名、在榜周数、走势点数，次排从总上榜播放开始，Phone 保持两列。
...
- 更新浏览器路由、年度交互、冷数据等待和 Settings 只读验收；详情最近播放分页补齐可访问名称，社区历史冷构建增加按参数 singleflight。
- 核心 API 性能门禁从两个热样本改为 21 个热样本；最终本地标准全栈命令以 `2212 passed`、前端 `561 passed`、API `128/128`、边界 `111/111`、40 组控件 0 violation 及 Chromium/Firefox/WebKit 全部通过结束。
- 补充本轮耗时诊断：最终一次约 27 分钟，后端全量测试是最慢单命令、浏览器验收是最大累计类别；四次全量后端重复共约 32 分 10 秒，并提出分阶段续跑、去除 Phase 5 重复覆盖和安全并行的缩时方案。
- 完整根因、真实库治理与验收边界见 [`reports/2026-08-24-fullstack-gate-repair.md`](reports/2026-08-24-fullstack-gate-repair.md)。
...
- 恰好跨一个开放周的精确尾部追加会有界重建新完成周的六套账本，复用旧历史周，并按稳定实体 ID 全局重算榜单摘要与 Power；当前开放周仍不发布，多周/历史变化或成本门禁失败时回退 shared-full。
- shared-full 与 delta 统一使用稳定实体 ID 归组，修复同名不同 ID 的专辑或艺人在榜单摘要中被合并的问题。
- 92,908 条真实库副本加 1 条尾部记录：同周 delta 1.093 秒、shared-full 26.416 秒；跨一周 delta 6.500 秒、shared-full 26.339 秒。两场景六套实体上下文与六套周账本均全列双向 `EXCEPT=0`，Phase D2 状态为 Pass。详见 [`reports/2026-08-23-incremental-import-phase-d2.md`](reports/2026-08-23-incremental-import-phase-d2.md)。
...
- Billboard 只为完整结束的周生成单曲、专辑和艺人榜；最新播放所在的覆盖边缘周不进入周榜、走势、纪录、Power Score 或 Year-End，最新几天的播放仍保留在全时统计中。
- 本次真实库诊断中，92,908 条播放的六套搜索快照同步尾段约 13 分钟；周五 12:00 边界下，2026-08-21 周只有约 12 小时、22 次播放，修复后最新榜单周应回退到 2026-08-14。完整证据见 [`reports/2026-08-23-import-and-complete-billboard-week-fix.md`](reports/2026-08-23-import-and-complete-billboard-week-fix.md)。

## 2026-08-23 — 封面缓存与导入后首屏性能

- 串流维护新增艺人批量元数据、精确名称搜索兜底和本地封面源同步；封面任务以实体类型和 ID 共同去重，下载校验、原子落盘、失败重试与真实失败状态补齐。
- 导入发布后先预热最新完整周榜和首页，再排队六套搜索快照；快照变体间只释放重型 Billboard/DB 帧，保留首页小快照，年度总结延后到快照完成后预热。
- 搜索快照统计改为向量聚合，只构建所需 weekly/summaries/power slices；真实副本默认变体由 42.3 秒降至 11.9 秒。精确口径版本改为按 key 计数，其他五个搜索变体不再反复使首页缓存失效。
- 真实库补下载后，与播放相关的专辑封面 2,930/2,930 文件可用；艺人署名 1,185/1,235 有可验证图片并已落盘，剩余 50 个无精确 Spotify 来源而保留未知。完整证据见 [`reports/2026-08-23-cover-cache-and-post-import-performance.md`](reports/2026-08-23-cover-cache-and-post-import-performance.md)。

## 2026-08-19 — 音乐详情按需加载与旧子页收口

- 歌曲、专辑、艺人详情改为摘要与播放统计并行加载，榜单概览、成员歌曲、成员专辑和专辑项目进入对应页签后再请求；`view=full` 保留完整响应兼容。
- 艺人成员歌曲改为服务端分页，长周榜历史按页挂载；播放次数路径跳过不需要的时长切片，并复用同一请求中的排行作用域，统计规则不变。
- 前端暂时移除歌曲歌词、专辑发行档案、艺人发行周期和艺人生涯页签；旧页签深链统一回到播放统计，不再触发歌词、增强、发行周期或 AI 后台请求。
- 真实本地数据库的 8 组详情、播放统计与成员排行 JSON 在优化前后完全一致；首屏摘要响应体由 6,361 / 35,753 / 183,808 B 降至 644 / 814 / 1,026 B。
- 后端 unit 1,092 项、contract 358 项、前端 547 项、生产构建、修改文件 lint、文档审计与真实浏览器详情验收通过；完整证据见 [`reports/2026-08-19-music-detail-performance-delivery.md`](reports/2026-08-19-music-detail-performance-delivery.md)。

## 2026-08-17 — GitHub Actions 工作流治理

- 将质量检查、生产发布、部署契约、镜像传输演练和一次性搜索快照引导改为职责清晰的工作流名称。
- 抽取可复用质量检查 workflow，生产发布额外执行 full/showcase/dual 三种部署模式校验。
- 为一次性搜索快照初始化保留手动 break-glass 入口，并要求显式输入确认值；同步更新测试、文档和 CI parity 检查。

## 2026-08-17 — 音乐查找展示收口

- 搜索结果不再向普通用户展示“匹配艺人/专辑/别名”“简繁匹配”“近似匹配”等内部诊断；匹配字段继续用于排序、高亮、测试和排障。
- 搜索结果名称、艺人/专辑副标题、封面辅助文字和最近查看列表统一跟随后台全局简繁体偏好；详情链接与本地保存值继续使用原始实体数据。

## 2026-08-17 — 歌曲、专辑、艺人详情统计修复

- 修复详情页总播放时长继承全库时长切片的问题；歌曲、专辑、艺人摘要、分布和时长排行现在都按实体范围重新生成时长帧。
- 修复艺人最近 50 次按 fan-out 行计数的问题，改为按稳定逻辑播放事件去重。
- 修复歌曲版本组、专辑发行组使用原始静态播放计数的问题，统一复用 Billboard `play_count` / `total_ms` 加权帧。
- 修复播放分析与详情页最近 4 周、最近 6 个月以服务器当天为结束日的问题，改为锚定最新有效数据日。
- 真实库复核 `vampire`、`Midnights`、Taylor Swift 的实体时长与播放时长排行；后端 API 集成 119 项、前端详情测试 11 项通过。
- 完整根因、修复范围与验证证据见 [`reports/2026-08-17-music-detail-statistics-fix-delivery.md`](reports/2026-08-17-music-detail-statistics-fix-delivery.md)。
...
- 查询加入 OpenCC 0.1.7 确定性简繁扩展、短 CJK n-gram，以及仅在主路径零命中时启用的最多 50 条
  trigram 候选池与有界编辑距离；匹配类型保留为内部排序、测试和诊断信息，不在消费界面展示。
...
- 新增 `logical_event_time_v2`：完整播放在累计收听达到歌曲时长时计数，余数播放在达到有效阈值时计数；每个展开事件拥有独立 `counted_at`、逻辑事件 ID、session ID 和时间质量标记。
- 播放次数与收听时长分轨归属：次数按 `counted_at` 进入自然日/月/年/Billboard 周，时长按推断收听区间切到小时、自然日和榜单周；跨边界查询保持分区不变性。
- Billboard 原始路径与预聚合统一使用 `play_count` / `total_ms` 双权重；四张聚合表先写临时影子表，再以单事务发布，失败时保留上一份完整快照。年度内容与过滤指纹升级到 `yearly_review_v2_13` / `yearly_review_filter_v2`，音乐档案过滤指纹升级到 `account_archive_filter_v2`。
- 真实库验收固定周五 12:00、动态阈值和 5 分钟边界：播放分析与 Billboard 在线 API 均返回 65,056 次，显式切换 15 分钟得到 65,032 次；迁移前备份与当前 `plays` 双向逐行差异为 0，单曲、单曲来源和艺人 raw/pre-agg 对账均为 0 差异。

## 2026-08-13 — 私人管理与公共只读双入口

- 在既有 Tailscale Serve 私人入口旁新增独立 `public-web` 网关：宿主只绑定 `127.0.0.1:3002`，由 Tailscale Funnel 8443 提供无需安装客户端的公开 HTTPS；3001 私人入口继续保留完整管理能力。
- 新增可信访问面能力发现与后端公共策略：设置、编辑、导入、AI、Spotify OAuth、歌词、元数据治理、后台任务和非白名单写操作在公网被拒绝，几个无副作用的 Billboard 对决 POST 保留。
- 公共年度总结改为精确缓存只读，公共封面不得触发外部搜索、数据库写入或后台下载；设置读取响应同时移除 Spotify 与 LLM 连接信息。
- Desktop/Compact/Phone 根据运行时能力隐藏 AI、设置、治理深链、歌词和导入入口，并显示“公开展示”标识；后端策略仍是最终安全边界。
- 部署、发布与验证脚本增加双 loopback 网关健康检查、写操作 403 门禁和 Funnel 配置，生产契约测试同步锁定端口、标头、挂载与公网边界。
...
- 新增 `/api/home/overview` 聚合契约，复用完整播放过滤指纹和歌曲/专辑/艺人权威归属；原始音乐源日期与有效播放日期分开表达，空数据、过滤后无有效播放和正常数据使用独立状态。
- 首页近期统计统一为以最新有效播放日锚定的最近 4 周；当前与对比窗口均为连续 28 个自然日，趋势补齐零播放日。首页近期入口携带同一起止日进入播放分析，直接打开播放分析仍默认全部时间。
- Billboard 与年度预览仅读取已存在的精确缓存，首页不触发冷构建；缓存键纳入预览 readiness，冷启动预热期间使用有界轮询，避免未就绪状态在当前页面固化。
- 删除重复的数据更新时间、音乐搜索、陈旧提醒及账号/AI/社区/更新数据快捷模块；App Shell 同时移除旧 Dashboard、周榜和总榜的无条件全局预取。
- 同步 OpenAPI 生成类型、首页测试、API/benchmark/route/control/cross-browser smoke marker、README、UI 规范与项目提示词；真实浏览器复核 Desktop/Phone 结构、最近 4 周数据对齐和首页到播放分析的跳转语义。

## 2026-08-13 — 账号中心重构为本地音乐档案

- 二级导航由“账号中心”统一改为“音乐档案”；封面移除收藏覆盖期、播放截止日和数据状态，只保留收藏歌曲、专辑、艺人和歌单四项概览。
- 人工验收收口章节文案：大标题不再附 subtitle 或方法说明；“从遇见到收藏 / 收藏之后 / 发现路径”改为“播放多久后收藏 / 收藏后再次播放 / 搜索与发现”，收藏前后 30 天改为直接并列次数，搜索漏斗只保留三步用户动作。
- Phone 删除含义不清的逐周“可观察”回访曲线，Desktop 与 Phone 的关系矩阵统一收敛为“近 90 天听过 / 没听 / 常听但未收藏”三项。
- Phone 人工验收收口：横向章节栏自动居中当前项并显示边缘提示，收藏生命力改为 2×2；长歌名与回归天数不再争抢列宽，完整收藏库增加直接跳页，播客/视频统一为更紧凑的封面 Top 3，正文辅助字号统一提升到可读层级；封面双行中文标题修复上下遮挡，并拉开概览数据与“开始翻阅”的间距。
- 人工验收继续收口封面与章节：封面四项概览改为紧凑 2×2 档案账格；收藏旅程用第 100 / 200 / 400 / 800 首里程碑替代重复的首末收藏；收藏前后 30 天对照改为纵向账格，“代表歌曲”明确为首次播放到收藏间隔最长的歌曲。
- 收藏后再次播放改为“7 天内又听 / 第 8–30 天仍听 / 半年后还在听 / 一年后还在听”四个互补指标；视频卡移除悬殊的音频时长对照，新增带封面和详情深链的本地可识别视频歌曲 Top 3。
- 人工验收继续调整版式：封面四项概览下移，Desktop 章节目录收窄；修复无序号实体行占错网格列导致沉睡歌曲名被挤压；播客卡与视频卡统一为标题条加 Top 3，并仅在本地已有节目封面时显示封面。
...
- `build_aggregations()` 完成后自动清理已注册运行时缓存，避免重建接口在同一进程继续返回旧的艺人榜快照。
- 真实数据库按周五 12:00、动态阈值开启重建；2023-06-30 Olivia Rodrigo 艺人榜从 83 修正为 159，`vampire` 单曲榜保持 118。
- 全量 9,969 个艺人-周组合原始/预聚合对账一致；13,000 行专辑榜原始/track-source 预聚合对账一致，新增逻辑事件一致性与专辑榜回归测试。

## 2026-08-12 — 年度总结 V2 完整内容重构

- Desktop/Compact 自定义年度总结重建为八章个人音乐年鉴：报告护照、播放/时长双榜、个人 Billboard 年度赛季、唯一时间线、关系故事、收听生活、年度纪录、品味迁移与完整榜单；官方 Wrapped 不变。
...
- 用户展示验收升级到 `content_version=yearly_review_v2_8`：年报移除防御性/工程化文案、方法页和大标题说明，六项年度 KPI 增加绝对同比与百分比，实体故事与完整榜单全面补齐封面/深链，月份只保留唯一时间线入口。
- Desktop/Compact 默认最近完整年度，当前年显示进行中；新增固定章节导航，海报按钮改为内容流操作，不再遮挡 KPI。probe v5 的 2023–2026 用户文案、封面、深链、YTD、重复与阶段检查为 0 issue；四年真实重算 10.65–16.54 秒、跨进程命中 10.20–21.07ms。
...
- 三类详情统计在手机端复用播放统计的时间范围/统计口径 Sheet，并将紧凑触发器固定在右下角；播放统计、播放排行、年度总结和播放记录移除重复页面标题，分析页时间筛选统一为右下角悬浮入口。
- 艺人详情的歌曲排行继续按有效合作署名 fan-out，专辑排行改为只统计该艺人拥有的 canonical album projects；合作曲位于其他艺人专辑时，不再把对方专辑计入艺人的专辑播放排行。
- 播放记录手机端改为顶部横向滑动栏目和共享 `MobileRecordTable`；统一单日巅峰、单日总量、个人王朝、长期陪伴、时间习惯、发现日等卡片的日期、字号、控件高度和指标层级，并移除重复标题与说明。
- 修复艺人“连续播放马拉松”被合作署名展开行错误打断的问题：连续性改按稳定逻辑播放事件判断；Midnights 的 452 次专辑连续播放对应 Taylor Swift 的完整 476 次艺人连续段。手机卡片直接显示“36 次连续播放”，不再重复显示“连续次数”。
- Billboard 记录中的艺人类事实统一读取有效合作署名，专辑所有权类事实继续使用主艺人；修复冠单、同时入榜、艺人生涯、自我接棒、双榜空降和全榜制霸等记录的合作艺人归属，并统一排名条最大值与共享移动榜单实现。
- 验证：后端相关测试 43 项、前端全量 61 files / 470 tests、最新移动记录定向 16 tests 与 TypeScript/Vite production build 通过；390/410px 实页为 0 横向溢出、0 console error/warning。
