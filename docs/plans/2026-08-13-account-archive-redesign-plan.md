...
> 日期：2026-08-13
> 状态：实施中；Phase 0、archive-overview、collection-journey、collection-cohorts、returns 已完成
...
| Settings | 如何导入、授权、治理和排除数据 | 音乐档案只显示简短数据状态并深链到 Settings |
...
- 800 首当前收藏；
- 27 个歌单；
- 收藏记录 2022-06—2026-05；
- 播放记录截至 2026-07；
- 762/800 首可与播放历史稳定关联。

封面右侧使用四张有角色的实体封面拼贴：最早收藏、长期回访、最近回归、当前沉睡。它们不是 Top 榜单，而是档案叙事入口。

不再展示头像、用户名、地区、关注数或人格徽章。页面底部以轻量文字显示“基于已导入数据 · 最近更新于 …”，并提供“查看数据状态”深链。
...
- 7 / 30 / 90 / 365 天回访率；
- 每个时距只使用具有完整观察窗的合格收藏作为分母；
- 展示合格样本，例如“90 天：722 首可观察”；
- 事件对齐曲线展示收藏前后每周有效播放，不再使用缺口窗口；
- 可选高级视图使用 Kaplan–Meier 处理到下次播放的右删失，普通 UI 只呈现“累计回访”。
...
```text
┌──────────────────────────────────────────────────────────┐
│ PERSONAL MUSIC ARCHIVE                     数据截至 07/24 │
│ 音乐档案                                                  │
│ 哪些音乐只是路过，哪些真正留了下来？                       │
│ [800 收藏] [27 歌单] [95.3% 可关联]      [四封面档案拼贴] │
└──────────────────────────────────────────────────────────┘

┌──────────────┬───────────────────────────────────────────┐
│ 00 档案封面   │ 01 收藏旅程：编辑式时间轴                  │
│ 01 收藏旅程   │ 02 从遇见到收藏：分布 + 实体故事            │
│ 02 遇见收藏   │ 03 收藏之后：事件对齐曲线 + 4 类关系矩阵      │
