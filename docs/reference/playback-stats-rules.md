...
当前缓存热响应预算为 250ms，未压缩主 JSON 预算为 512 KiB，真实重算预算为 30 秒。关系历史中的首次播放与回归前末次播放日期必须按实体类型一次性聚合，不得在逐实体循环中反复扫描和转换完整历史；年度播放纪录应复用编排层已经建立的年度事件与实体帧。最终 artifact 可以压缩持久化，但必须使用独立 sidecar SQLite，完整 cache key 继续包含数据库 revision、过滤指纹、策略和元数据 revision；不得因缓存写入改变统计主库 revision，不得用旧 key 或近似 key 返回 stale 报告。持久缓存损坏时必须自动视为 miss 并同步重算，设置变更、流式导入与应用启动可在后台预建最新年份。性能优化不得改变统计口径或让浏览器静默吞掉本地 API 超时。2023–2026 同进程真实重算为 9.95–15.94 秒，跨进程持久命中为 11.31–23.68ms，主报告与完整纪录目录指纹逐年一致。
