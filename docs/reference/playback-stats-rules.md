...
`/analysis/records` 使用当前过滤参数下的有效逻辑播放事件。需要个人历史序号的播放里程碑必须在完整 standalone 历史上计算，再根据页面周期筛选里程碑发生日期；年度总结注入的年度预加载帧不自动宣称为 lifetime。艺人 fan-out 必须保留有效 credit 的 `role`，合作艺人记录只统计 `role = 'featured'`。

记录列表必须先在完整候选集上计算精确指标并排序，再应用 Top N。主要指标相同时按模块定义的业务二级指标排序，仍相同时使用 canonical stable key 或规范化文本；不得使用 DataFrame 输入顺序、index 或截断后的偶然顺序。毫秒、比率和序号使用未取整值，小时和百分比取整只用于展示。

当前页面的统一规则如下：单日次数/冠军次数使用播放次数，其次为总毫秒；时长榜使用总毫秒，其次为播放次数；连续记录使用长度，其次为段内或实体总毫秒/播放次数，再按结束日期和 stable key；周期冠军先在每个日/月/年周期内按播放次数、毫秒和 stable key 选唯一 winner，再汇总实体榜。发现、合作曲、同名异曲和专辑全碟回放分别按数量、播放次数、毫秒和稳定实体键裁决。深夜与其他比率记录按未取整比率排序，走势图本身按时间正序展示，最高周期再使用同一精确比率规则。

播放记录行可以返回 `total_ms` 作为精确排序证据；`total_hours`、`secondary_value` 等显示字段不得反向作为精度排序输入。前端的播放次数/时长切换和最高深夜周期选择必须沿用后端相同的精确指标与后续稳定键。

播放记录的页面文案属于用户解释层：主标题已经足够明确的板块可以不显示 subtitle；保留 subtitle 时，只描述用户能直接理解的比较对象、范围或结果。不得把 `run`、`idxmax`、fan-out、数据帧、字段缺失、发行日资格判断等内部处理术语或技术性排除条件直接放进用户副标题；这类规则保留在本参考文档和交付报告中。歌曲、专辑和艺人记录应尽可能复用统一的封面占位和既有详情深链，视觉展示不能改变统计事实或排序。
...
冠军传承的单曲替换行同时展示前冠单、新冠单及艺人名称，歌曲名和艺人名均保持既有详情深链；专辑替换行继续展示专辑名和艺人名。替换箭头只表示前后关系，必须与右侧新实体保持明确视觉留白。阻挡王的被阻挡歌曲标签应带小型歌曲封面，缺图时使用统一占位，且不改变原有歌曲详情链接。
