...
- 前端只在没有可用事实时展示准备或不可用状态；已有 LKG 或保留内容时静默刷新，以稳定容器的 `aria-busy` 或屏幕阅读器状态表达，不插入会引发布局位移的可见维护提示，也不向普通用户暴露“发布”等后台术语。
...
公开端不开放维护 POST，时间选择器只暴露自动维护的三个常用范围；若直接访问尚无结果的深链，返回结构化 unavailable。已有 LKG 时保留旧事实并静默刷新，不把后台维护状态显示成面向用户的页面内容。
