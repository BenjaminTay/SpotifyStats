...
## 7. 2026-08-30 Records 页面文案与展示细节收口（后续补充）

### 7.1 用户化文案

- `冠军圣殿`、`爆发时刻`、`持久传奇`、`奇趣纪录` 等主标题已经能够表达板块含义的页面移除冗余导语。
- 其余 Records 卡片的 subtitle 改为直接说明用户能看到的结果，例如“同一周上榜歌曲最多的艺人”“在第二名停留最久，却从未夺冠”。
- 移除 `Peak #2` 等榜单内部术语和过度技术化说明，避免把实现细节、数据资格条件或缺失值处理写入用户副标题。

### 7.2 冠军传承与阻挡王展示

- 冠军传承的单曲模式现在同时显示前冠单、新冠单及艺人名称；歌曲名和艺人名均保持详情深链，专辑模式继续显示专辑名和艺人名。
- 前后实体之间使用统一箭头图标；桌面端箭头列宽调整为 72px，手机端列间距调整为 16px，使箭头与右侧新实体封面保持稳定留白。
- 阻挡王的被阻挡歌曲标签增加 20px 小封面，并保留歌曲详情链接；缺失封面时沿用统一占位。
- `SectionHeader.subtitle` 改为可选字段，`CoverImg` 支持小尺寸变体，避免为了填充布局而强制生成无意义副标题或重复实现封面逻辑。

### 7.3 回归与状态

`frontend/src/tests/mobile-m4-pages.test.tsx` 覆盖可选板块 subtitle、冠军传承单曲艺人链接、箭头列宽和阻挡歌曲封面。前端定向测试 13/13 通过，生产构建和 `git diff --check` 通过；本补充不改变 Billboard 统计、候选集、排序或接口契约。
