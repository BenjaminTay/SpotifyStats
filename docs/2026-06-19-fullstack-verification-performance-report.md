...
- 后端全量测试通过：`671 passed, 2 warnings`
- 前端测试与构建通过：`131 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `301 passed`，contract `170 passed`，前端 `131 passed`，`npm run build` 通过
...
- 跨浏览器冒烟通过：`scripts/frontend_cross_browser_smoke.mjs --include-detail-routes` 使用 Playwright Chromium / Firefox / WebKit（Safari-family）覆盖 6 个核心路由 + 5 个动态详情路由 × 桌面/390px 移动端；本轮 dev server route-marker 详情覆盖 PASS 3/3 浏览器引擎，生产 `vite preview` 产物已有核心路由 + 4 个非破坏性交互 PASS 3/3，生产 preview 通过 Playwright `route.fetch`/`route.fulfill` 代理后端响应，避免 4173 非 CORS 白名单 origin 造成假阴性或弱证据
...
- 跨浏览器 probe：新增 `scripts/frontend_cross_browser_smoke.mjs`，通过 Python Playwright API 覆盖 Chromium、Firefox、WebKit（Safari-family）。每个浏览器引擎默认执行 6 个核心路由 × 桌面/390px 移动端 marker 检查，以及 `analysis-tabs`、`billboard-routing`、`ai-insights-tabs`、`theme-toggle` 4 个非破坏性交互；`--include-detail-routes` 会从 `/api/billboard/entity-lists` 与 `/api/community/feed` 动态追加歌曲/专辑/艺人/社区帖子/社区账号 5 个详情路由并应用业务 marker。生产 preview 用 `--api-base-url` 通过 Playwright `route.fetch` + `route.fulfill` 代理 `/api` 与 `/covers` 响应，并在关闭页面前等待 network idle，避免 4173 非 CORS 白名单和代理请求未完成造成假阴性。dev server route-marker 详情覆盖 PASS 3/3 浏览器引擎，生产 `vite preview` 核心路由/交互产物 PASS 3/3。该探针需要可导入 `playwright.sync_api` 的 Python，可用 `PYTHON_PLAYWRIGHT=/path/to/python` 或 `--python` 指定。
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`671 passed, 2 warnings` | 已自动验证 |
