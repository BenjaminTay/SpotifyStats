...
- Runtime resource probe 已补齐：`scripts/runtime_resource_probe.py` 可按 backend/frontend/preview URL 定位监听进程树，输出本地 CPU/RSS Markdown 与 JSON，并支持总量/服务级资源预算；本轮本机 dev 服务快照为总 RSS `719.8MB`、总 CPU `5.0%`，`fullstack_verification_check.sh --resource-snapshot` 可把该快照和可选 `--resource-max-total-*` 预算纳入非破坏性验收矩阵
- 快速开始一键启动冒烟通过：`scripts/quickstart_smoke.py --json-output /tmp/spotify_quickstart_timing.json` 自动启动或复用后端 8000 与前端 5173，验证 `/api/health`、`/docs`、前端 HTML shell 与 Vite `/api/health` 代理后清理自启进程；本轮本机复用后端、自启前端，总耗时 `2454.6ms`，backend health `2.4ms`，docs `2.7ms`，frontend shell `428.7ms`，frontend API proxy `5.2ms`
- 全栈非破坏性验收矩阵已有完整 PASS 记录：`scripts/fullstack_verification_check.sh` 在本地 dev server 串起 backend full、pre-commit、Phase 5、OpenAPI operation audit、OpenAPI parameter boundary audit、API smoke/boundary、benchmark、前端 route/interaction/chart/control-inventory/long-list/cross-browser smoke；2026-06-19 曾追加 `--preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals` 跑通生产 `vite preview` smoke 与 dev/prod Web Vitals。本轮 preview 转发、预算门禁与详情路由覆盖补强后，单独复跑 dev route smoke PASS 48/48、prod-preview route smoke PASS 48/48、interaction smoke PASS 6/6、control inventory dev/prod-preview PASS 36/36、cross-browser Chromium/Firefox/WebKit 完整 dev route-marker + core-interactions PASS 3/3、prod-preview core-interactions PASS 3/3，且脚本已把 preview route/interaction/chart/control-inventory/long-list/cross-browser smoke 与 Web Vitals 的 `/api`、`/covers` 请求转发到后端；Web Vitals probe 可用 LCP/CLS/TBT/资源数量/encoded 体积/横向滚动溢出预算参数在明显性能回退、资源回弹或移动端溢出时非零退出，全栈聚合入口也可用 `--web-vitals-max-lcp-ms` / `--web-vitals-max-cls` / `--web-vitals-max-tbt-ms` / `--web-vitals-max-resource-count` / `--web-vitals-max-encoded-resource-kb` / `--web-vitals-max-scroll-overflow-px` 传递同一预算；`--quickstart-preflight` 可在已启动服务上复用 quickstart health/docs/frontend shell/API proxy 验证并写出 timing JSON；dev/prod route smoke 已将 console warning 纳入失败门禁；脚本会在激活 `.venv` 前自动检测可导入 `playwright.sync_api` 的 Python，避免跨浏览器 smoke 误用无 Playwright 的 venv Python
- pre-commit 通过：ruff、ruff format、mypy、detect-secrets 全部通过
- 浏览器路由冒烟通过：`scripts/frontend_route_smoke.mjs --include-detail-routes` 覆盖 19 个默认主路由 + 5 个动态详情路由（歌曲/专辑/艺人/社区帖子/社区账号）× 1280px 桌面/390px 移动端共 48 个组合，dev server 与生产 `vite preview` 均 PASS 48/48，console error/warning、page error 与页面级横向滚动均为 0，并通过路由业务内容 marker 防止只加载导航壳的误判
- 浏览器交互冒烟通过：`scripts/frontend_interaction_smoke.mjs` 覆盖分析页 tab、Billboard 子路由/浏览器前进后退、AI Insights 报告/问答 tab（浏览器内读取 `/api/settings` 后按 LLM 已配置/未配置分支验证）、Settings 过滤/显示偏好控件、Settings 数据导入区非破坏性卡片/按钮检查与主题切换共 6 个非破坏性场景；dev server 与生产 `vite preview` 产物均 PASS 6/6，生产 preview 通过 `--api-base-url` 转发同源 API/cover 请求，console error/warning、page error 与页面级横向滚动均为 0
...
- 控件库存冒烟通过：`scripts/frontend_control_inventory_smoke.mjs --include-detail-routes` 覆盖 13 个默认路由 + 5 个动态详情路由 × 桌面/390px 移动端，检查可见交互控件缺少可访问名称、嵌套交互控件、disabled 仍 tabbable、输入控件无标签和重复 id；首轮暴露 Billboard/Records/AllTime/WeekSelector 纯图标分页按钮无名称、Settings Slider 内部 range input 无名称以及 Base UI hidden input 误伤，生产 preview 复跑进一步暴露 `/analysis/charts` 封面图片链接空 `alt` 造成 50 个无名链接；修复后 dev server 覆盖 36 组合 / 1821 个可见控件 / 0 violation，生产 `vite preview` 覆盖 36 组合 / 1763 个可见控件 / 0 violation
- 跨浏览器冒烟通过：`scripts/frontend_cross_browser_smoke.mjs --include-detail-routes` 使用 Playwright Chromium / Firefox / WebKit（Safari-family）覆盖 6 个核心路由 + 5 个动态详情路由 × 桌面/390px 移动端，并复跑 6 个核心交互（Analysis、Billboard、AI Insights、Settings 控件、Settings 数据导入区、主题切换）；本轮 dev server 完整 route-marker + core-interactions 覆盖 PASS 3/3 浏览器引擎，生产 `vite preview` 产物 core-interactions PASS 3/3，生产 preview 通过 Playwright `route.fetch`/`route.fulfill` 代理后端响应，避免 4173 非 CORS 白名单 origin 造成假阴性或弱证据
...
- 可复跑 API 边界 probe 通过：`scripts/api_boundary_probe.py` 覆盖 85 个非破坏性 GET 边界，包含越界参数、非法 path/query 类型、非法 path/entity、特殊字符查询，验证预期 422/200、`X-Request-ID`、无 500，且 422 响应带 FastAPI validation detail
- OpenAPI parameter boundary audit 通过：`scripts/openapi_parameter_boundary_audit.py --json-output /tmp/spotify_openapi_parameter_boundary_audit.json` 核算当前 59 个参数边界义务，归属为 36 个 `boundary_probe`、16 个 `string_resilience_probe`、7 个 `controlled_stateful_or_external`，`unaccounted=0`
- OpenAPI operation audit 通过：`scripts/openapi_operation_audit.py --json-output /tmp/spotify_openapi_operation_audit.json` 核算当前 122 paths / 134 operations，归属为 95 个 `safe_get_smoke`、30 个 `targeted_contract`、9 个 `controlled_external_or_stateful`，`unaccounted=0`
...
| P3 | TTL 缓存失效只清空条目、不重置 hit/miss 统计 | 设置、导入或命名空间失效后，`/api/admin/cache-stats` 仍会展示旧统计周期的命中/未命中计数，削弱缓存观测和调优判断 | `ttl_cached.cache_clear()` 改为在同一把锁内清空条目并归零 hit/miss，新增 unit 测试覆盖装饰器直接清理与 `CacheManager.invalidate(namespace)` 路径 |
| P3 | Job Queue `stop()` 不等待运行中任务完成，运行中 stop 后同队列 restart 可能吃到旧 sentinel | FastAPI lifespan shutdown 名义上是 graceful，但旧实现会立即返回；正在执行的封面/百科/歌词后台任务可能在进程退出前被 daemon 线程截断，同一个 `JobQueue` 实例重启时还可能被遗留 sentinel 干扰 | `stop()` 现在会唤醒 worker、等待线程结束、清理停止后的队列残留；worker 对 sentinel 也调用 `task_done()`；新增 unit 测试先红灯复现 stop 不等待，再覆盖运行中 stop 后 restart 能继续处理新任务 |
| P3 | 导入进度回调可把负数或大于 1 的 `progress_pct` 暴露给状态端点 | Settings 数据导入进度条直接使用 `progress_pct * 100%`，越界进度会导致 UI 宽度异常，削弱数据导入 API 状态契约 | `_progress_cb()` 写入 job 状态前将进度 clamp 到 `[0.0, 1.0]`；新增 contract 测试先红灯复现 `-0.5/1.5` 泄露，再验证状态端点保持 0-1 |
...
| P3 | 前端 route smoke 只检查 root 文本长度，可能把“只有导航壳、业务内容尚未渲染”的页面误判为通过 | 生产 preview 冷跑时 `/billboard/records` 桌面在 2.5s 采样点曾只有导航文本，但旧探针仍 PASS；这会削弱前端零缺陷验证证据 | 为 19 个默认路由增加业务内容 marker，默认等待提高到 5s，并提供 `--disable-route-markers` 作为自定义路由逃生口；生产 preview marker smoke 含动态详情路由复跑 PASS 48/48，Records 桌面 root text 从导航壳 45 提升到 3141 |
| P3 | 音乐实体详情与社区详情页未纳入默认浏览器路由 smoke | API smoke 覆盖了音乐详情和社区 post/account 接口，但 route smoke 只打开主页面，无法证明 `/music/tracks/:id`、`/music/albums/:albumName`、`/music/artists/:artistName`、`/community/post/:postId` 和 `/community/account/:handle` 在真实浏览器、桌面/移动视口下无白屏、无 console warning、无横向溢出 | `frontend_route_smoke.mjs` 新增 `--include-detail-routes`，从 `/api/billboard/entity-lists` 与 `/api/community/feed` 动态解析 5 个真实详情样本并应用详情页业务 marker；`fullstack_verification_check.sh` 的 dev/preview route smoke 默认启用该参数；本机 dev server 单命令复跑 PASS 48/48，含 `/music/tracks/4451`、`/music/albums/Midnights?artist=Taylor%20Swift`、`/music/artists/Taylor%20Swift`、`/community/post/7171ac0efaa4`、`/community/account/%40chartdata` 两视口全绿 |
...
| P3 | 前端交互 smoke 的 AI Insights 分支依赖瞬时页面文案，且 Settings 数据导入区未纳入默认交互覆盖 | `/ai-insights` 在 settings 查询返回前会短暂渲染“AI 功能尚未配置”空态；本地已配置 LLM 时，旧脚本可能把该瞬时空态当最终状态，随后等待错误文案导致默认 smoke 假阴性。同时 Settings 数据导入区只被 route smoke 看到标题，缺少卡片/启动按钮可见性证据 | `frontend_interaction_smoke.mjs` 新增浏览器内 `fetchLlmAvailability()`，通过 `/api/settings` 的 `llm_enabled` + `has_llm_key` 决定已配置/未配置分支；新增 `settings-data-import` 场景，验证串流数据/账号数据卡片、数据库记录数文案、账号导入说明和 2 个导入动作按钮可见但不启动任务；新增 unit 护栏并复跑 dev/prod-preview interaction smoke PASS 6/6，console error/warning/page error 与横向溢出均为 0 |
| P3 | 可见交互控件缺少系统性库存检查，图标按钮、封面链接和 slider 内部 input 可无名称地进入页面 | route/interaction smoke 能证明代表性路径可用，但不能自动发现纯图标分页按钮、周选择按钮、Base UI slider input、封面图片链接或隐藏表单输入的可访问名称/嵌套/重复 id 问题；这会削弱“所有按钮、下拉菜单、表单交互”要求的基础证据 | 新增 `scripts/frontend_control_inventory_smoke.mjs`，通过 headless Chrome CDP 覆盖 13 默认路由 + 5 动态详情路由 × 桌面/390px 移动端，检查 `missing accessible name`、`nested interactive control`、`disabled but tabbable`、`input without label`、`duplicate id`；修复 WeekSelector、AllTime/Records/StatsTables、音乐详情分页图标按钮 `aria-label`，Slider wrapper 将 `aria-label` 传给 Base UI Thumb，`CoverCell` 支持 `label` 并为排行榜/详情封面传入实体名，脚本排除 Base UI `*-hidden-input`、同一路由视口切换前复位并在 marker 等待失败时重试一次；dev 36 组合 / 1821 控件 / 0 violation，prod-preview 36 组合 / 1763 控件 / 0 violation，并已接入 fullstack 聚合入口 |
| P3 | 生产 `vite preview` 下 interaction/cross-browser smoke 未显式代理 `/api` 与 `/covers` | `vite preview` 不继承 Vite dev proxy，后端 CORS 白名单也不包含 4173；如果 smoke 只验证静态壳或空状态，可能把 API 未命中误判为生产预览可用。Playwright `route.continue_(url=...)` 还会触发跨 origin 语义，不能等价模拟同源 dev proxy | `frontend_interaction_smoke.mjs` 新增 CDP Fetch `--api-base-url` 转发；`frontend_cross_browser_smoke.mjs` 新增 `--api-base-url` 并用 Playwright `route.fetch` + `route.fulfill` 代理响应，关闭页面前等待 network idle 防止代理请求被打断；`fullstack_verification_check.sh` 的 preview interaction/cross-browser 调用接入 `--preview-api-url`；新增 unit 护栏并实跑 production preview interaction PASS 6/6、Chromium/Firefox/WebKit PASS |
...
| P3 | Web Vitals probe 默认路由缺少 `/analysis/charts` | 报告和全栈脚本按 6 条 Web Vitals 路由描述性能采样，但直接运行默认 `frontend_web_vitals_probe.mjs` 只覆盖 5 条路由，漏掉图表分析页默认性能证据 | 默认 `DEFAULT_ROUTES` 补齐 `/analysis/charts`，新增 unit 护栏锁定默认覆盖；默认命令现在与全栈矩阵的 6 路由口径一致 |
| P3 | Web Vitals probe 只能人工阅读 Markdown，缺少 LCP/CLS/TBT 回归门禁 | 性能采样已有前后数据，但不能像 API benchmark 一样在明显超预算时非零退出；后续 CI 或本地增强矩阵仍可能漏掉前端 lab 指标回退 | `frontend_web_vitals_probe.mjs` 新增 `--max-lcp-ms`、`--max-cls`、`--max-tbt-ms`，任一路由/视口缺失或超过预算会保留 Markdown/JSON 报告并输出 `Web Vitals budget failures` 后以退出码 1 失败；新增 unit 护栏锁定 CLI 与失败行为，文档给出保守预算示例 `LCP<=3000ms / CLS<=0.01 / TBT<=100ms` |
| P3 | 全栈聚合入口未转发 Web Vitals 预算参数 | 单独 Web Vitals probe 已能作为 LCP/CLS/TBT 门禁，但 `fullstack_verification_check.sh --web-vitals` 仍只能采样，无法在一键验收矩阵中复用相同预算 | `fullstack_verification_check.sh` 新增 `--web-vitals-max-lcp-ms`、`--web-vitals-max-cls`、`--web-vitals-max-tbt-ms` 与同名环境变量，通过 `run_web_vitals_probe()` 同时传给 dev 和 preview Web Vitals；新增 unit 护栏锁定 CLI、环境变量和 preview helper 转发 |
| P3 | Web Vitals 缺少资源数量、资源体积与横向滚动溢出预算门禁 | 账号页资源已从约 250 requests / 25MB 收敛，移动端也要求 0 横向溢出；但原预算只覆盖 LCP/CLS/TBT，如果请求数、encoded resource KB 或 scroll width 回弹而核心指标暂时未恶化，聚合验收仍可能漏掉网络资源或响应式回归 | `frontend_web_vitals_probe.mjs` 新增 `--max-resource-count`、`--max-encoded-resource-kb` 与 `--max-scroll-overflow-px`，`fullstack_verification_check.sh` 新增 `--web-vitals-max-resource-count` / `--web-vitals-max-encoded-resource-kb` / `--web-vitals-max-scroll-overflow-px` 与同名环境变量；stderr 进度输出同步展示 `Resources=count/kb`，Markdown/JSON 输出保留 scroll width 与 overflow；新增 unit 护栏锁定 CLI、预算失败路径和 fullstack 透传，文档给出保守预算示例 `resource count<=120 / encoded resources<=11000KB / scroll overflow<=0px` |
| P3 | Runtime resource probe 只有 RSS 快照，缺少 CPU 与预算失败路径 | 目标要求同时降低 CPU、内存、网络资源占用；RSS-only 报告能人工阅读，但无法把明显 CPU/RSS 回弹纳入一键验收矩阵 | `runtime_resource_probe.py` 新增进程树 `%CPU` 采样、总 CPU/RSS 汇总、总量与服务级预算参数；`fullstack_verification_check.sh` 新增 `--resource-max-total-rss-mb` / `--resource-max-total-cpu-percent` 与同名环境变量。本轮本机 dev 服务快照：backend `579.9MB / 5.0%`、frontend `139.9MB / 0.0%`、总计 `719.8MB / 5.0%`；极低预算探针会以退出码 1 失败并列出超标项 |
...
| P3 | 全栈聚合入口 route smoke 未把 console warning 作为失败 | 单独 route smoke 可用 `--fail-on-console-warning` 验证零警告，但一键 fullstack 矩阵没有传递该参数；后续如果页面新增 warning，聚合验收可能仍然通过 | `fullstack_verification_check.sh` 的 dev 与 production preview route smoke 都传递 `--fail-on-console-warning`，新增 unit 护栏锁定聚合脚本 wiring，文档命令统一更新为零警告口径 |
| P3 | 快速开始指南缺少可复跑的一键启动与启动/首屏时序证据 | `fullstack_verification_check.sh` 能串起完整验收矩阵，但要求调用者已手动启动后端和前端；“完整执行快速开始指南，确保项目可一键启动”和“记录初始基准数据”仍主要依赖人工照命令操作与阅读日志 | 新增 `scripts/quickstart_smoke.py`，自动启动或复用后端 8000 与前端 5173，验证 backend health、FastAPI docs、前端 HTML shell 与 Vite API proxy，并清理脚本自启进程；新增 `--json-output` 写出 total/backend docs/frontend shell/proxy 时序、body bytes、request id 状态与服务复用状态；新增 `--require-running` 让聚合矩阵可在不启动/清理服务的情况下复用同一验证；`fullstack_verification_check.sh --quickstart-preflight --quickstart-json ...` 会调用该模式并写 timing JSON；新增 unit 护栏锁定 CLI、启动命令、warmup 默认关闭、进程清理、docs marker、JSON timing report 与 require-running 不启动服务语义 |
...
| P3 | Firefox/Safari-family 兼容性缺少可复跑自动证据 | 之前的浏览器证据主要来自 Chromium CDP；“确保 Chrome、Firefox、Safari 正常”的要求只能算部分覆盖 | 新增 `scripts/frontend_cross_browser_smoke.mjs`，用 Python Playwright API 跑 Chromium、Firefox、WebKit（Safari-family），覆盖 6 个核心路由 × 桌面/390px 移动端、横向溢出、console/page error，以及 Analysis/Billboard/AI Insights/Settings/主题切换 6 个核心交互；新增 `--include-detail-routes` 后可从本地 API 动态追加歌曲/专辑/艺人/社区帖子/社区账号 5 个详情路由，本轮 dev server 完整 route-marker + core-interactions 覆盖 PASS 3/3 浏览器引擎，生产 preview core-interactions 三引擎 PASS |
| P3 | 跨浏览器 core interactions 未覆盖 Settings 控件/数据导入区，AI Insights 分支也依赖瞬时页面文案 | Chromium CDP interaction 已覆盖 Settings，但 Firefox/WebKit 路径只跑 Analysis、Billboard、AI Insights 和主题切换；同时 `/ai-insights` 在 settings 查询返回前可能短暂显示未配置空态，三引擎 smoke 也可能被该瞬态误导 | `frontend_cross_browser_smoke.mjs` 的 core interactions 增加 `settings-controls` 与 `settings-data-import`，并复用 `/api/settings` 的 `llm_enabled` + `has_llm_key` 判断 AI 已配置/未配置分支；新增 unit 护栏并实跑 dev 完整 cross-browser smoke（3 引擎 × 11 路由 × 2 视口 + 6 交互）PASS，生产 preview core interactions 三引擎 PASS |
| P3 | 生产 preview 跨浏览器代理在页面关闭期间可能抛 `TargetClosedError`，WebKit 快速导航会产生 Vite preload 诊断 warning | Settings 场景会触发设置请求；页面关闭时 `route.fetch(url=rewritten)` 仍可能在飞，导致事件监听器异常拖垮后续浏览器流程。WebKit 在 `vite preview` 快速路由烟测中还会输出资源 preload 未及时使用的浏览器诊断 warning，污染应用 warning 门禁 | preview 请求代理改为 best-effort：`route.fetch`/`route.fulfill` 失败时尝试 `route.abort()` 并吞掉关闭期异常；console guard 仅过滤已知 WebKit/Vite preload 诊断，其他 warning/error 仍失败；新增 unit 护栏并复跑生产 preview core interactions 三引擎 PASS |
...
| P3 | API 参数边界覆盖仍分散在少量 contract 测试中 | “空值、超长值、非法值、特殊字符”边界要求缺少一条可独立复跑的只读探针；happy-path API smoke 无法证明代表性 422 响应都带 request id 与 validation detail | `scripts/api_boundary_probe.py` 扩展到 85 个非破坏性 GET 边界：limit/offset/top_n/merge_level/Billboard top-N/week offset/release-cycle window/significance 越界、非法 path/query 类型、invalid/empty entity、特殊字符搜索；新增 unit/contract 护栏，锁定预期 422/200、`X-Request-ID`、无 500 和 FastAPI validation detail |
| P3 | OpenAPI 参数边界义务缺少机器可读归属 | boundary probe 扩展后仍需要确认每个带边界/模式约束的 query 参数、type-only integer query 和 integer path 参数都有明确证据；否则新增参数可能只出现在 `/docs`，没有边界 probe 或显式排除 | 新增 `scripts/openapi_parameter_boundary_audit.py`，读取 `app.openapi()` 并核算 59 个参数边界义务：36 个归属到 schema boundary case，16 个归属到 string resilience case，7 个外部态或生成态参数显式归属为 controlled stateful/external，`unaccounted=0`；`fullstack_verification_check.sh` 已纳入该 audit |
| P3 | OpenAPI 非 GET 与外部态端点覆盖归属缺少机器可读核算 | GET smoke 已能证明只读路径，但 POST/PUT/PATCH/DELETE 与外部态 GET 主要散落在 contract 或已知排除说明中；新增路由可能没有明确验证归属就混入 `/docs` | 新增 `scripts/openapi_operation_audit.py`，读取 `app.openapi()` 并将 134 个 operation 分类为 safe GET smoke、targeted contract 或 controlled stateful/external，默认输出 Markdown，可用 `--json-output` 写机器可读清单，若新增 operation 未分类则非零失败；`fullstack_verification_check.sh` 已纳入该 audit |
...
| P3 | 全栈验证命令分散，容易漏跑 API、browser、benchmark 或 pre-commit 中的一类 | 交付要求需要后端、API、前端、浏览器、性能和质量检查都有明确入口；旧状态只能依赖报告中的长命令清单，复跑成本高 | 新增 `scripts/fullstack_verification_check.sh` 聚合非破坏性验收矩阵：backend full、pre-commit、Phase 5、OpenAPI operation audit、OpenAPI parameter boundary audit、API smoke/boundary、benchmark fail-on-slow、前端 route/interaction/chart/long-list/cross-browser smoke；preview 和 Web Vitals 通过 `--preview-url` / `--web-vitals` 显式启用；新增 unit 护栏和 `sh -n` 语法验证 |
...
- API boundary probe：`scripts/api_boundary_probe.py` 在 TestClient 本地应用实例通过 85/85 个非破坏性 GET 边界，覆盖 Analysis/Dashboard/Leaderboard/Community/Library/Version Merge/Billboard/Release Cycle/Music Entity/Lyrics/Chat/AI Insights 的越界查询参数、非法 path/query 类型、非法 path/entity、空/超长字符串与特殊字符搜索；每个响应均验证预期状态、`X-Request-ID`、无 500，422 响应验证 FastAPI validation detail。
- OpenAPI parameter boundary audit：`scripts/openapi_parameter_boundary_audit.py` 直接读取 `backend.main.app.openapi()`，核算 bounded/pattern query 参数、type-only integer query 参数、integer path 参数以及 string path/query 参数的 59 个参数边界义务，当前结果为 `boundary_probe=36`、`string_resilience_probe=16`、`controlled_stateful_or_external=7`、`unaccounted=0`；`--json-output /tmp/spotify_openapi_parameter_boundary_audit.json` 会写出每个 obligation 的 location、name、signature、occurrence_count、category、evidence 与 rationale，后续新增未分类参数边界会使脚本失败。
- OpenAPI operation audit：`scripts/openapi_operation_audit.py` 直接读取 `backend.main.app.openapi()`，核算 122 paths / 134 operations，当前结果为 `safe_get_smoke=95`、`targeted_contract=30`、`controlled_external_or_stateful=9`、`unaccounted=0`；`--json-output /tmp/spotify_openapi_operation_audit.json` 会写出每个 operation 的 method、path、category、evidence 与 rationale，后续新增未分类 operation 会使脚本失败。
...
- Cache Manager unit probe：`backend/tests/unit/test_cache.py` 覆盖 `ttl_cached.cache_clear()` 与 `CacheManager.invalidate(namespace)` 对 TTL 条目和 hit/miss 统计的同步清理语义；新增红绿验证先暴露旧行为 `hits/misses` 未归零，再修复后复跑 `12 passed`。
- Job Queue unit probe：`backend/tests/unit/test_job_queue.py` 覆盖 enqueue/process、重复 pending 去重、singleton、graceful stop、运行中任务 stop 等待，以及运行中 stop 后同队列 restart 不被旧 sentinel 干扰；新增红绿验证先暴露旧 `stop()` 立即返回，再修复后复跑 `7 passed`。
...
- Import job probe：contract 测试用同步 fake thread 验证 `/api/import/streaming` 与 `/api/import/account` 的 job_id 返回、进度回调、进度 clamp、完成状态、account 嵌套结果摘要，以及 streaming 导入异常时的 error 状态。
...
- 前端 route smoke probe：新增 `scripts/frontend_route_smoke.mjs`，通过 headless Chrome CDP 覆盖 `/`、Analysis、5 个分析重定向别名、Yearly Review、Billboard 4 页、Community、AI Insights、Account、Settings 共 19 个主路由；`--include-detail-routes` 会从本地 API 动态追加歌曲/专辑/艺人/社区帖子/社区账号 5 个详情路由，合计桌面/移动 48 个组合；默认 5s 等待并检查每个默认路由的业务内容 marker，避免只加载导航壳就通过。dev server 与生产 `vite preview` 当前均 PASS 48/48，console error/warning/page error 全 0，scroll overflow 全 0px。首轮探针暴露 AI Insights 嵌套按钮与重复 key console error；生产 preview 补充探针暴露 `/billboard/records` 桌面 2.5s 导航壳误判风险，增强 marker 后复跑全绿。
- 前端交互 probe：新增 `scripts/frontend_interaction_smoke.mjs`，通过 headless Chrome CDP 执行 6 个非破坏性交互场景：`analysis-tabs` 在 `/analysis/stats` 与 `/analysis/charts` 间点击切换；`billboard-routing` 执行 `/billboard` → `/billboard/number-ones` → `/billboard/all-time` → `/billboard/records` 并验证浏览器后退/前进；`ai-insights-tabs` 先读取 `/api/settings` 的 `llm_enabled` 与 `has_llm_key`，配置 LLM 时继续覆盖月报/年度叙事和问答 UI，未配置时验证空状态；`settings-controls` 验证 Settings 关键区块、过滤/动态阈值 switch、中文显示偏好 localStorage 与 Spotify 连接按钮状态；`settings-data-import` 验证串流数据/账号数据导入卡片、数据库记录数文案、账号导入说明和 2 个导入动作按钮可见但不启动任务；`theme-toggle` 验证白日/夜晚按钮同步 DOM class 与 `localStorage.theme`。脚本支持 `--api-base-url`，用于生产 `vite preview` 页面 + 8000 后端 API/cover 的 CDP Fetch 转发；dev server 与生产 `vite preview` 产物均 PASS 6/6，console error/warning/page error 全 0，scroll overflow 全 0px。
...
- 前端控件库存 probe：新增 `scripts/frontend_control_inventory_smoke.mjs`，通过 headless Chrome CDP 检查可见交互控件是否有可访问名称、是否存在嵌套交互控件、disabled 控件是否仍可 tab、输入控件是否有标签、控件 id 是否重复；支持 `--include-detail-routes` 从 `/api/billboard/entity-lists` 与 `/api/community/feed` 追加歌曲/专辑/艺人/社区帖子/社区账号详情样本，支持 `--api-base-url` 跑生产 preview + 8000 API。首轮暴露 88 个 violation，集中在 Billboard/Records/AllTime/WeekSelector 纯图标分页按钮、Settings Slider 内部 range input 和 Base UI hidden input 误伤；生产 preview 复跑进一步暴露 `/analysis/charts` 封面图片链接空 `alt` 造成 50 个无名链接；修复后 dev server 36 组合 / 1821 个可见控件 / 0 violation，生产 `vite preview` 36 组合 / 1763 个可见控件 / 0 violation。
- 跨浏览器 probe：新增 `scripts/frontend_cross_browser_smoke.mjs`，通过 Python Playwright API 覆盖 Chromium、Firefox、WebKit（Safari-family）。每个浏览器引擎默认执行 6 个核心路由 × 桌面/390px 移动端 marker 检查，以及 `analysis-tabs`、`billboard-routing`、`ai-insights-tabs`、`settings-controls`、`settings-data-import`、`theme-toggle` 6 个非破坏性交互；`ai-insights-tabs` 会读取 `/api/settings` 判断 LLM 已配置/未配置分支。`--include-detail-routes` 会从 `/api/billboard/entity-lists` 与 `/api/community/feed` 动态追加歌曲/专辑/艺人/社区帖子/社区账号 5 个详情路由并应用业务 marker。生产 preview 用 `--api-base-url` 通过 Playwright `route.fetch` + `route.fulfill` 代理 `/api` 与 `/covers` 响应，关闭期代理异常会 best-effort abort，避免 4173 非 CORS 白名单、代理请求未完成或页面已关闭造成假阴性；WebKit/Vite 快速导航下的 preload 浏览器诊断会被单独过滤，其它 warning/error 仍失败。dev server 完整 route-marker + core-interactions 覆盖 PASS 3/3 浏览器引擎，生产 `vite preview` core-interactions PASS 3/3。该探针需要可导入 `playwright.sync_api` 的 Python，可用 `PYTHON_PLAYWRIGHT=/path/to/python` 或 `--python` 指定。
...
- 快速开始 probe：新增 `scripts/quickstart_smoke.py`，默认复用 README 的 8000/5173 端口，若服务未运行则以 `SPOTIFY_STATS_WARMUP=0` 自动启动后端 `uvicorn backend.main:app` 和前端 `npm run dev -- --host 127.0.0.1 --port 5173 --strictPort`；依次验证后端 `/api/health` 带 `X-Request-ID`、`/docs` 含稳定 `swagger-ui` marker、前端 HTML shell 含 `id="root"`、Vite `/api/health` 代理带 `X-Request-ID`，最后清理自启进程。`--json-output` 可写出机器可读时序报告；`--require-running` 可只复用已运行服务并在缺失时失败，供全栈聚合矩阵的 `--quickstart-preflight` 使用；本轮本机复用后端、自启前端，总耗时 `2454.6ms`，backend health `2.4ms`，docs `2.7ms`，frontend shell `428.7ms`，frontend API proxy `5.2ms`。
- Runtime resource probe：新增 `scripts/runtime_resource_probe.py`，通过本地 `lsof` 定位 backend/frontend/preview 监听进程，并用 `pgrep` + `ps` 汇总包含子进程的 CPU/RSS；默认输出 Markdown，`--json-output` 写机器可读报告，`--fail-on-missing` 可在服务缺失时作为门禁，`--max-total-rss-mb` / `--max-total-cpu-percent` 与服务级 `--max-service-*` 可在资源超预算时非零退出；`fullstack_verification_check.sh --resource-snapshot` 会在 benchmark 后采集 backend/frontend/preview CPU/RSS 快照。
- 全栈聚合 probe：`sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals` 此前在本地后端 8000 + Vite dev 5173 + `vite preview` 4173 环境完整通过，覆盖 backend full `616 passed`、pre-commit、Phase 5 unit `261` / contract `155` / frontend `130` / build、API smoke `91/91`、API boundary `19/19`、Provider error contract、Infrastructure response-model contract、当时默认 route smoke、interaction `5/5`、chart `3/3`、long-list `6/6`、cross-browser Chromium/Firefox/WebKit `PASS`，以及 dev/prod Web Vitals lab；本轮 response-model/API smoke、Billboard enrichment 降级契约、OpenCC 按需加载、路由 smoke 覆盖、preview 转发、route warning 门禁、详情路由覆盖、Web Vitals 预算门禁、runtime resource probe、quickstart timing、OpenAPI operation audit 与 OpenAPI parameter boundary audit 收口后已单独复跑 backend full `685 passed`、unit `314 passed` / contract `171` / frontend `131` / build、OpenAPI operation audit `134 operations / 0 unaccounted`、OpenAPI parameter boundary audit `59 obligations / 0 unaccounted`、API boundary `85/85`、quickstart JSON smoke `2454.6ms`、dev route smoke `48/48`、dev interaction smoke `6/6`、dev control inventory `36/36` / `1821 controls` / `0 violation`、dev cross-browser route-marker detail smoke `3/3`、prod-preview route smoke `48/48`、prod-preview interaction `6/6`、prod-preview control inventory `36/36` / `1763 controls` / `0 violation`、prod-preview cross-browser Chromium/Firefox/WebKit `PASS`、prod-preview Web Vitals `/account` 390px PASS 以及 API smoke `96/96`。`fullstack_verification_check.sh` 当前 dev/preview route smoke、control inventory 与 cross-browser route-marker smoke 会通过 `--include-detail-routes` 追加音乐/社区详情页，preview route/interaction/chart/control-inventory/long-list/cross-browser smoke 和 Web Vitals 均会通过 `--preview-api-url` 显式分流 `/api` 与 `/covers` 请求，并可用 `--quickstart-preflight` 复用已运行服务验证 quickstart health/docs/frontend shell/API proxy，可用 `--web-vitals-max-*` 把 LCP/CLS/TBT/资源数量/encoded 体积/横向滚动溢出预算纳入聚合矩阵；开启 `--resource-snapshot` 时还会在 benchmark 后采集 backend/frontend/preview 监听进程树 CPU/RSS 快照，并可用 `--resource-max-total-rss-mb` / `--resource-max-total-cpu-percent` 设置聚合资源预算。
- Web Vitals lab probe（Vite dev server + headless Chrome + CDP）：默认覆盖 6 路由 × 桌面/390px 移动端（含 `/analysis/charts`）；增强矩阵复跑采样 CLS 全部 0，合成点击 FID 0.8-4.0ms，TBT 全部 0ms，非账号页 LCP 416-868ms，账号页 LCP 2,188ms（桌面）/ 2,264ms（移动）。`frontend_web_vitals_probe.mjs` 现支持 `--api-base-url`，生产 `vite preview` 采样可将 `/api` 与 `/covers` 转发到 8000 后端，避免只测静态壳；同时支持 `--max-lcp-ms`、`--max-cls`、`--max-tbt-ms`、`--max-resource-count`、`--max-encoded-resource-kb`、`--max-scroll-overflow-px`，全栈聚合入口支持对应 `--web-vitals-max-*` 参数，可把采样结果升级为 LCP/CLS/TBT/资源数量/encoded 体积/横向滚动溢出 lab 预算门禁。本轮 production preview `/account` 390px 单路由复跑 PASS，LCP 464ms、CLS 0、TBT 0ms、encoded resources 400.4KB，后端日志出现 request id；已有补充采样资源体积更接近交付产物：非账号页 LCP 376-864ms，账号页 LCP 2,140ms（桌面）/ 2,168ms（移动），CLS/TBT 全部 0。
...
- 文档同步：README、AGENTS、CLAUDE、backend/CLAUDE、frontend/CLAUDE 已更新 2026-06-19 验证报告、API smoke / API boundary / OpenAPI operation audit / OpenAPI parameter boundary audit / Provider error / Billboard enrichment degradation / Infrastructure/Settings/Spotify auth/Account-center/Core-stats/Remaining-JSON response-model / API benchmark / fullstack verification / quickstart smoke / Web Vitals 预算门禁 / route smoke / interaction smoke / chart interaction smoke / control inventory smoke / cross-browser smoke / long-list smoke / CI parity 探针、Power Score 向量化、Behavior API 参数收窄、OAuth PKCE 本地合同验证、移动端横向滚动护栏、pre-commit 范围与最新测试基线。
...
> 可选门禁：追加 `--max-lcp-ms 3000 --max-cls 0.01 --max-tbt-ms 100 --max-resource-count 120 --max-encoded-resource-kb 11000 --max-scroll-overflow-px 0` 可在任一路由/视口超过预算时非零退出。
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`685 passed, 2 warnings` | 已自动验证 |
| OpenAPI/核心 API 覆盖 | 122 paths / 134 operations schema 存在；96 个可复跑只读请求覆盖 Dashboard、Billboard、Analysis、Community、Lyrics 未命中、Settings 缺失 profile、缺失封面、AI Insights、Account、Spotify status/data；OpenAPI GET 核算 0 unaccounted；OpenAPI operation audit 核算 134 operations：95 safe GET smoke / 30 targeted contract / 9 controlled stateful-external / 0 unaccounted；OpenAPI parameter boundary audit 核算 59 个参数边界义务：36 boundary probe / 16 string resilience probe / 7 controlled stateful-external / 0 unaccounted；85 个 API boundary GET 覆盖越界参数、非法 path/query 类型、非法 path/entity、空/超长字符串与特殊字符查询；Provider error contract 覆盖六类上游失败的结构化响应分层；Billboard enrichment contract 覆盖 4 个外部增强 GET 在 Wiki 普通异常下返回空增强而非 500，并验证 ProviderError 仍走结构化 503；基础设施 response-model contract 覆盖 6 个启动/缓存/导入/Job 状态端点；Settings mutation response-model contract 覆盖 6 个设置写端点；Spotify auth response-model contract 覆盖 7 个 OAuth/Web API JSON 端点；Account-center response-model contract 覆盖 12 个账号中心/画像 GET 端点；Core-stats response-model contract 覆盖 6 个核心统计 GET 端点；Remaining JSON response-model contract 覆盖 release-cycle compare 与 lyrics 3 个 JSON 端点，剩余未声明 response_model 的 `/api` 路由数为 1（唯一剩余项为 Spotify callback 显式 RedirectResponse） | 已覆盖只读核心路径、参数边界归属、Provider 失败响应、Billboard enrichment 降级、基础设施响应契约、Settings 关键 mutation、Spotify auth JSON、账号中心/画像、核心统计与剩余 JSON 响应契约；破坏性或外部态端点已有机器可读归属，不在本地真实状态上逐一实打 |
...
| 前端路由与响应式 | `scripts/frontend_route_smoke.mjs --include-detail-routes` 覆盖 19 主路由 + 5 个动态详情路由 × 桌面/390px 移动端，无页面错误、无 console error/warning、无横向溢出，并检查业务内容 marker；dev server 与生产 `vite preview` 均 PASS 48/48；`scripts/frontend_cross_browser_smoke.mjs --include-detail-routes` 额外覆盖 Chromium/Firefox/WebKit 的 6 核心路由 + 5 动态详情路由 × 2 视口，本轮 dev server 完整 route-marker + core-interactions PASS 3/3，生产 preview 通过 `--api-base-url` 代理 API/cover 响应；图表入口由架构护栏防止回退到完整 ECharts/OpenCC 默认包；Web Vitals lab 覆盖 6 路由 × 2 视口，生产 preview 可用 `--api-base-url` 代理后端数据；音乐详情隐藏 tab 图表挂载由架构护栏防回归 | 已自动验证主路径、音乐/社区详情页与三浏览器引擎 smoke |
| 前端交互 / 本地 mutation | `scripts/frontend_interaction_smoke.mjs` 覆盖分析页 tab、Billboard 子路由/前进后退、AI Insights 报告/问答 tab（按 `/api/settings` 判断 LLM 已配置/未配置分支）、Settings 过滤/显示偏好控件、Settings 数据导入区卡片/动作按钮可见性与主题切换，dev/prod preview 均 PASS 6/6，生产 preview 通过 `--api-base-url` 代理 API/cover 响应；`scripts/frontend_control_inventory_smoke.mjs` 覆盖 13 默认路由 + 5 动态详情路由 × 2 视口的可见交互控件命名、嵌套、tabbable、表单标签和重复 id，dev `1821 controls / 0 violation`、prod-preview `1763 controls / 0 violation`；`scripts/frontend_chart_interaction_smoke.mjs` 覆盖 ECharts hover tooltip、legend toggle、dataZoom drag，dev/prod preview 均 PASS 3/3；`scripts/frontend_cross_browser_smoke.mjs` 在 Chromium/Firefox/WebKit 复跑同一组 6 个核心交互，dev 完整矩阵 PASS 3/3，prod-preview core-interactions PASS 3/3；`scripts/frontend_long_list_smoke.mjs` 覆盖 Records/AllTime/Community Feed/RecentPlays/SavedTracks/PersonalRankTable 长列表分页或分段渲染，dev/prod preview 均 PASS 6/6；Chat CRUD、Settings 更新、LLM profile CRUD/apply、清翻译缓存、Import job 启动/状态在 contract 临时环境自动验证；`PUT /api/settings` 现在返回完整脱敏 Settings payload，避免设置页 Query 缓存被局部响应覆盖 | 已自动验证非破坏性主交互、Settings 关键控件、Settings 数据导入 UI 可见性、三浏览器核心交互、可见控件基础可访问性、代表性图表交互和 6 个长列表窗口变化；破坏性按钮提交流与全部图表实例仍未逐项人工穷尽 |
| 快速开始 / 一键启动 | `scripts/quickstart_smoke.py --json-output /tmp/spotify_quickstart_timing.json` 自动启动或复用后端 8000 与前端 5173，验证 `/api/health`、`/docs`、前端 HTML shell 与 Vite `/api/health` 代理，且退出后不遗留脚本自启进程；本轮本机复用后端、自启前端，总耗时 `2454.6ms`，backend health `2.4ms`，docs `2.7ms`，frontend shell `428.7ms`，frontend API proxy `5.2ms` | 已自动验证 README 默认端口的一键启动、前后端代理联通与机器可读启动/首屏时序 |
| 本地 CI / 全栈验收流程 | `scripts/ci_baseline_parity.py` 提取 GitHub Actions baseline 并确认 `scripts/phase5_check.sh` 覆盖 unit、contract、ruff、frontend test、frontend build；`phase5_check.sh` 已内置该 parity 护栏；`scripts/fullstack_verification_check.sh` 已在 dev server + 生产 `vite preview` + Web Vitals 模式完整串起 backend full、pre-commit、Phase 5、OpenAPI operation audit、OpenAPI parameter boundary audit、API probes、benchmark 与前端 smoke，且可把 LCP/CLS/TBT/资源数量/encoded 体积/横向滚动溢出预算传入 Web Vitals 子探针；可选 `--resource-snapshot` 将 backend/frontend/preview CPU/RSS 写入 JSON，并支持总 RSS/CPU 预算 | 已自动验证本地最低矩阵与 CI baseline 不漂移；全栈非破坏性验收入口已实跑通过 |
| 性能优化 | records profile、API 冷/热请求、`benchmark_api.py` slow endpoint summary/JSON report、runtime resource CPU/RSS snapshot、build/import、bundle chunk、dev/prod-preview Web Vitals lab 与账号页资源/TBT 均有前后对比；preview Web Vitals 可通过 `--api-base-url` 采样真实后端数据页，并可通过 LCP/CLS/TBT/资源数量/encoded 体积/横向滚动溢出预算参数作为 lab 回归门禁；runtime resource probe 本机 dev 快照为总 RSS `719.8MB`、总 CPU `5.0%`，保守预算示例为 RSS <= `1200MB` / CPU <= `200%` | 已量化关键瓶颈并补齐可选前端 lab 与进程 CPU/RSS 预算证据入口；仍缺真实 RUM 与生产静态托管 Lighthouse |
...
.venv/bin/python scripts/api_boundary_probe.py
.venv/bin/pytest backend/tests/unit/test_openapi_operation_audit_script.py -q
.venv/bin/python scripts/openapi_operation_audit.py --json-output /tmp/spotify_openapi_operation_audit.json
.venv/bin/pytest backend/tests/unit/test_openapi_parameter_boundary_audit_script.py -q
.venv/bin/python scripts/openapi_parameter_boundary_audit.py --json-output /tmp/spotify_openapi_parameter_boundary_audit.json
...
.venv/bin/pytest backend/tests/unit/test_frontend_control_inventory_smoke_script.py -q
...
.venv/bin/pytest backend/tests/unit/test_quickstart_smoke_script.py -q
.venv/bin/pytest backend/tests/unit/test_runtime_resource_probe_script.py -q
.venv/bin/python scripts/quickstart_smoke.py --timeout-sec 90 --json-output /tmp/spotify_quickstart_timing.json
