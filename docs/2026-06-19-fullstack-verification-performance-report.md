...
- 后端全量测试通过：`653 passed, 2 warnings`
- 前端测试与构建通过：`131 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `283 passed`，contract `170 passed`，前端 `131 passed`，`npm run build` 通过
...
- Runtime resource probe 已补齐：`scripts/runtime_resource_probe.py` 可按 backend/frontend/preview URL 定位监听进程树，输出本地 RSS Markdown 与 JSON；`fullstack_verification_check.sh --resource-snapshot` 可把该快照纳入非破坏性验收矩阵
...
- Runtime resource probe：新增 `scripts/runtime_resource_probe.py`，通过本地 `lsof` 定位 backend/frontend/preview 监听进程，并用 `pgrep` + `ps` 汇总包含子进程的 RSS；默认输出 Markdown，`--json-output` 写机器可读报告，`--fail-on-missing` 可在服务缺失时作为门禁；`fullstack_verification_check.sh --resource-snapshot` 会在 benchmark 后采集 backend/frontend/preview RSS 快照。
- 全栈聚合 probe：`sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals` 此前在本地后端 8000 + Vite dev 5173 + `vite preview` 4173 环境完整通过，覆盖 backend full `616 passed`、pre-commit、Phase 5 unit `261` / contract `155` / frontend `130` / build、API smoke `91/91`、API boundary `19/19`、Provider error contract、Infrastructure response-model contract、当时默认 route smoke、interaction `5/5`、chart `3/3`、long-list `6/6`、cross-browser Chromium/Firefox/WebKit `PASS`，以及 dev/prod Web Vitals lab；本轮 response-model/API smoke、Billboard enrichment 降级契约、OpenCC 按需加载、路由 smoke 覆盖、preview 转发、route warning 门禁、Web Vitals 预算门禁与 runtime resource probe 收口后已单独复跑 backend full `653 passed`、Phase 5 unit `283` / contract `170` / frontend `131` / build、dev route smoke `38/38`、prod-preview route smoke `38/38`、prod-preview interaction `5/5`、prod-preview cross-browser Chromium/Firefox/WebKit `PASS`、prod-preview Web Vitals `/account` 390px PASS 以及 API smoke `96/96`。`fullstack_verification_check.sh` 当前 dev/preview route smoke 会把 console warning 作为失败，preview route/interaction/chart/long-list/cross-browser smoke 和 Web Vitals 均会通过 `--preview-api-url` 显式分流 `/api` 与 `/covers` 请求，并可用 `--web-vitals-max-*` 把 LCP/CLS/TBT/资源数量/encoded 体积预算纳入聚合矩阵；开启 `--resource-snapshot` 时还会在 benchmark 后采集 backend/frontend/preview 监听进程树 RSS 快照。
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`653 passed, 2 warnings` | 已自动验证 |
...
| 本地 CI / 全栈验收流程 | `scripts/ci_baseline_parity.py` 提取 GitHub Actions baseline 并确认 `scripts/phase5_check.sh` 覆盖 unit、contract、ruff、frontend test、frontend build；`phase5_check.sh` 已内置该 parity 护栏；`scripts/fullstack_verification_check.sh` 已在 dev server + 生产 `vite preview` + Web Vitals 模式完整串起 backend full、pre-commit、Phase 5、API probes、benchmark 与前端 smoke，且可把 LCP/CLS/TBT/资源数量/encoded 体积预算传入 Web Vitals 子探针；可选 `--resource-snapshot` 将 backend/frontend/preview RSS 写入 JSON | 已自动验证本地最低矩阵与 CI baseline 不漂移；全栈非破坏性验收入口已实跑通过 |
| 性能优化 | records profile、API 冷/热请求、`benchmark_api.py` slow endpoint summary/JSON report、runtime resource RSS snapshot、build/import、bundle chunk、dev/prod-preview Web Vitals lab 与账号页资源/TBT 均有前后对比；preview Web Vitals 可通过 `--api-base-url` 采样真实后端数据页，并可通过 LCP/CLS/TBT/资源数量/encoded 体积预算参数作为 lab 回归门禁 | 已量化关键瓶颈并补齐可选前端 lab 与进程 RSS 预算证据入口；仍缺真实 RUM 与生产静态托管 Lighthouse |
...
.venv/bin/python scripts/runtime_resource_probe.py --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --json-output /tmp/spotify_runtime_resources.json
...
sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals --resource-snapshot --web-vitals-max-lcp-ms 3000 --web-vitals-max-cls 0.01 --web-vitals-max-tbt-ms 100 --web-vitals-max-resource-count 120 --web-vitals-max-encoded-resource-kb 11000
...
10. 运行 `sh scripts/phase5_check.sh`，确认最低矩阵仍全绿；需要完整非破坏性矩阵时，运行 `sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173`，需要 Web Vitals 与 RSS 预算证据时追加 `--web-vitals --resource-snapshot --web-vitals-max-lcp-ms 3000 --web-vitals-max-cls 0.01 --web-vitals-max-tbt-ms 100 --web-vitals-max-resource-count 120 --web-vitals-max-encoded-resource-kb 11000`。
