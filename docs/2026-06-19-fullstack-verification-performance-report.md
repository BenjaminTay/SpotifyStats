...
- 后端全量测试通过：`655 passed, 2 warnings`
- 前端测试与构建通过：`131 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `285 passed`，contract `170 passed`，前端 `131 passed`，`npm run build` 通过
...
- 快速开始一键启动冒烟通过：`scripts/quickstart_smoke.py` 自动启动后端 8000 与前端 5173，验证 `/api/health`、`/docs`、前端 HTML shell 与 Vite `/api/health` 代理后清理自启进程；本地复跑约 7s 通过，Vite ready `197ms`，退出后 8000/5173 无残留监听
...
| P3 | 快速开始指南缺少可复跑的一键启动证据 | `fullstack_verification_check.sh` 能串起完整验收矩阵，但要求调用者已手动启动后端和前端；“完整执行快速开始指南，确保项目可一键启动”仍主要依赖人工照命令操作 | 新增 `scripts/quickstart_smoke.py`，自动启动或复用后端 8000 与前端 5173，验证 backend health、FastAPI docs、前端 HTML shell 与 Vite API proxy，并清理脚本自启进程；新增 unit 护栏锁定 CLI、启动命令、warmup 默认关闭、进程清理和 docs marker |
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`655 passed, 2 warnings` | 已自动验证 |
...
| 快速开始 / 一键启动 | `scripts/quickstart_smoke.py` 自动启动或复用后端 8000 与前端 5173，验证 `/api/health`、`/docs`、前端 HTML shell 与 Vite `/api/health` 代理，且退出后不遗留 8000/5173 监听；本地复跑约 7s PASS | 已自动验证 README 默认端口的一键启动与前后端代理联通 |
...
.venv/bin/python scripts/quickstart_smoke.py --timeout-sec 90
...
1. 运行 `.venv/bin/python scripts/quickstart_smoke.py --timeout-sec 90`，确认默认 8000/5173 可自动启动、API docs 可访问、Vite `/api` 代理可用，且退出后无残留监听。
