...
- 后端全量测试通过：`669 passed, 2 warnings`
- 前端测试与构建通过：`131 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `299 passed`，contract `170 passed`，前端 `131 passed`，`npm run build` 通过
...
- 全栈聚合 probe：`sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals` 此前在本地后端 8000 + Vite dev 5173 + `vite preview` 4173 环境完整通过，覆盖 backend full `616 passed`、pre-commit、Phase 5 unit `261` / contract `155` / frontend `130` / build、API smoke `91/91`、API boundary `19/19`、Provider error contract、Infrastructure response-model contract、当时默认 route smoke、interaction `5/5`、chart `3/3`、long-list `6/6`、cross-browser Chromium/Firefox/WebKit `PASS`，以及 dev/prod Web Vitals lab；本轮 response-model/API smoke、Billboard enrichment 降级契约、OpenCC 按需加载、路由 smoke 覆盖、preview 转发、route warning 门禁、详情路由覆盖、Web Vitals 预算门禁、runtime resource probe、quickstart timing、OpenAPI operation audit 与 OpenAPI parameter boundary audit 收口后已单独复跑 backend full `669 passed`、Phase 5 unit `299` / contract `170` / frontend `131` / build、OpenAPI operation audit `134 operations / 0 unaccounted`、OpenAPI parameter boundary audit `59 obligations / 0 unaccounted`、API boundary `85/85`、quickstart JSON smoke `2454.6ms`、dev route smoke `48/48`、prod-preview route smoke `38/38`、prod-preview interaction `5/5`、prod-preview cross-browser Chromium/Firefox/WebKit `PASS`、prod-preview Web Vitals `/account` 390px PASS 以及 API smoke `96/96`。`fullstack_verification_check.sh` 当前 dev/preview route smoke 会把 console warning 作为失败并通过 `--include-detail-routes` 追加音乐/社区详情页，preview route/interaction/chart/long-list/cross-browser smoke 和 Web Vitals 均会通过 `--preview-api-url` 显式分流 `/api` 与 `/covers` 请求，并可用 `--web-vitals-max-*` 把 LCP/CLS/TBT/资源数量/encoded 体积预算纳入聚合矩阵；开启 `--resource-snapshot` 时还会在 benchmark 后采集 backend/frontend/preview 监听进程树 CPU/RSS 快照，并可用 `--resource-max-total-rss-mb` / `--resource-max-total-cpu-percent` 设置聚合资源预算。
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`669 passed, 2 warnings` | 已自动验证 |
