...
> 可选门禁：追加 `--max-lcp-ms 3000 --max-cls 0.01 --max-tbt-ms 100` 可在任一路由/视口超过预算时非零退出。
...
| 本地 CI / 全栈验收流程 | `scripts/ci_baseline_parity.py` 提取 GitHub Actions baseline 并确认 `scripts/phase5_check.sh` 覆盖 unit、contract、ruff、frontend test、frontend build；`phase5_check.sh` 已内置该 parity 护栏；`scripts/fullstack_verification_check.sh` 已在 dev server + 生产 `vite preview` + Web Vitals 模式完整串起 backend full、pre-commit、Phase 5、API probes、benchmark 与前端 smoke，且可把 LCP/CLS/TBT 预算传入 Web Vitals 子探针 | 已自动验证本地最低矩阵与 CI baseline 不漂移；全栈非破坏性验收入口已实跑通过 |
| 性能优化 | records profile、API 冷/热请求、`benchmark_api.py` slow endpoint summary/JSON report、build/import、bundle chunk、dev/prod-preview Web Vitals lab 与账号页资源/TBT 均有前后对比；preview Web Vitals 可通过 `--api-base-url` 采样真实后端数据页，并可通过 LCP/CLS/TBT 预算参数作为 lab 回归门禁 | 已量化关键瓶颈并补齐可选前端 lab 预算门禁；仍缺真实 RUM 与生产静态托管 Lighthouse |
...
sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals --web-vitals-max-lcp-ms 3000 --web-vitals-max-cls 0.01 --web-vitals-max-tbt-ms 100
...
- Web Vitals 已用 headless Chrome lab probe 采集 Vite dev server 与本地 `vite preview` 生产构建的 LCP/CLS/合成 FID/TBT，preview 采样可用 `--api-base-url` 代理后端数据，预算参数可防明显回退；仍未采集真实用户 RUM、Firefox/WebKit Web Vitals，也未在生产静态托管/CDN/HTTPS 环境跑 Lighthouse。
...
10. 运行 `sh scripts/phase5_check.sh`，确认最低矩阵仍全绿；需要完整非破坏性矩阵时，运行 `sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173`，需要 Web Vitals 预算门禁时追加 `--web-vitals --web-vitals-max-lcp-ms 3000 --web-vitals-max-cls 0.01 --web-vitals-max-tbt-ms 100`。
