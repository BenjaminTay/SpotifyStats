...
| P3 | Web Vitals 缺少资源数量与资源体积预算门禁 | 账号页资源已从约 250 requests / 25MB 收敛，但原预算只覆盖 LCP/CLS/TBT；如果请求数或 encoded resource KB 回弹而核心指标暂时未恶化，聚合验收仍可能漏掉网络资源回归 | `frontend_web_vitals_probe.mjs` 新增 `--max-resource-count` 与 `--max-encoded-resource-kb`，`fullstack_verification_check.sh` 新增 `--web-vitals-max-resource-count` / `--web-vitals-max-encoded-resource-kb` 与同名环境变量；stderr 进度输出同步展示 `Resources=count/kb`；新增 unit 护栏锁定 CLI、预算失败路径和 fullstack 透传，文档给出保守预算示例 `resource count<=120 / encoded resources<=11000KB` |
...
> 可选门禁：追加 `--max-lcp-ms 3000 --max-cls 0.01 --max-tbt-ms 100 --max-resource-count 120 --max-encoded-resource-kb 11000` 可在任一路由/视口超过预算时非零退出。
...
| 本地 CI / 全栈验收流程 | `scripts/ci_baseline_parity.py` 提取 GitHub Actions baseline 并确认 `scripts/phase5_check.sh` 覆盖 unit、contract、ruff、frontend test、frontend build；`phase5_check.sh` 已内置该 parity 护栏；`scripts/fullstack_verification_check.sh` 已在 dev server + 生产 `vite preview` + Web Vitals 模式完整串起 backend full、pre-commit、Phase 5、OpenAPI operation audit、OpenAPI parameter boundary audit、API probes、benchmark 与前端 smoke，且可把 LCP/CLS/TBT/资源数量/encoded 体积预算传入 Web Vitals 子探针；可选 `--resource-snapshot` 将 backend/frontend/preview CPU/RSS 写入 JSON，并支持总 RSS/CPU 预算 | 已自动验证本地最低矩阵与 CI baseline 不漂移；全栈非破坏性验收入口已实跑通过 |
| 性能优化 | records profile、API 冷/热请求、`benchmark_api.py` slow endpoint summary/JSON report、runtime resource CPU/RSS snapshot、build/import、bundle chunk、dev/prod-preview Web Vitals lab 与账号页资源/TBT 均有前后对比；preview Web Vitals 可通过 `--api-base-url` 采样真实后端数据页，并可通过 LCP/CLS/TBT/资源数量/encoded 体积预算参数作为 lab 回归门禁；runtime resource probe 本机 dev 快照为总 RSS `719.8MB`、总 CPU `5.0%`，保守预算示例为 RSS <= `1200MB` / CPU <= `200%` | 已量化关键瓶颈并补齐可选前端 lab 与进程 CPU/RSS 预算证据入口；仍缺真实 RUM 与生产静态托管 Lighthouse |
