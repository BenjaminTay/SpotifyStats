...
- 浏览器路由冒烟通过：`scripts/frontend_route_smoke.mjs --include-detail-routes` 覆盖 19 个默认主路由 + 5 个动态详情路由（歌曲/专辑/艺人/社区帖子/社区账号）× 1280px 桌面/390px 移动端共 48 个组合，console error/warning、page error 与页面级横向滚动均为 0；生产 `vite preview` 产物已有主路由 PASS 38/38，并通过路由业务内容 marker 防止只加载导航壳的误判
