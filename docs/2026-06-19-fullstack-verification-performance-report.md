...
- 后端全量测试通过：`627 passed, 2 warnings`
- 前端测试与构建通过：`130 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `261 passed`，contract `166 passed`，前端 `130 passed`，`npm run build` 通过
...
- 可复跑只读 API smoke 通过：`scripts/api_smoke_probe.py` 覆盖 93 个本地只读 GET 请求，全部返回预期状态并带 `X-Request-ID`；OpenAPI GET 核算 `92/104 covered, 12 excluded, 0 unaccounted`
...
- API smoke：`scripts/api_smoke_probe.py` 在真实本地库通过 93/93 个本地只读 GET 请求，覆盖 Dashboard、Analysis、Timeline、Leaderboard、Billboard、Release Cycle、Music Entity、Lyrics 未命中、Community、Version Merge、Account、AI Insights、Chat、Admin、Job、Spotify status/data，并逐项验证 `X-Request-ID`；OpenAPI GET 核算 `92/104 covered, 12 excluded, 0 unaccounted`，默认列表排除 AI 生成、enrichment、OAuth callback/login、live playback 与静态封面等会触发外部网络、浏览器态或非稳定本地 artifact 的路径。
...
| 目标项 | 当前证据 | 状态 |
| --- | --- | --- |
| 后端现有测试全量通过 | `pytest backend/tests/ -q`：`627 passed, 2 warnings` | 已自动验证 |
| OpenAPI/核心 API 只读覆盖 | 122 paths / 134 operations schema 存在；93 个可复跑只读请求覆盖 Dashboard、Billboard、Analysis、Community、Lyrics 未命中、AI Insights、Account、Settings、Spotify status/data；OpenAPI GET 核算 0 unaccounted；19 个 API boundary GET 覆盖代表性越界参数、非法 path/entity 与特殊字符查询；Provider error contract 覆盖六类上游失败的结构化响应分层；基础设施 response-model contract 覆盖 6 个启动/缓存/导入/Job 状态端点；Settings mutation response-model contract 覆盖 6 个设置写端点；Spotify auth response-model contract 覆盖 7 个 OAuth/Web API JSON 端点；Account-center response-model contract 覆盖 12 个账号中心/画像 GET 端点；Core-stats response-model contract 覆盖 6 个核心统计 GET 端点；Remaining JSON response-model contract 覆盖 release-cycle compare 与 lyrics 3 个 JSON 端点，剩余未声明 response_model 的 `/api` 路由数为 1（唯一剩余项为 Spotify callback 显式 RedirectResponse） | 已覆盖只读核心路径、代表性参数边界、Provider 失败响应、基础设施响应契约、Settings 关键 mutation、Spotify auth JSON、账号中心/画像、核心统计与剩余 JSON 响应契约；其它 mutation/外部依赖端点未逐一实打 |
...
8. 运行 `.venv/bin/python scripts/api_smoke_probe.py` 和 `.venv/bin/python scripts/api_boundary_probe.py`，确认 93 个只读 GET 与 19 个边界 GET 全绿。
