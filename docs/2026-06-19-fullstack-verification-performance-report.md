...
- 后端全量测试通过：`666 passed, 2 warnings`
- 前端测试与构建通过：`131 passed`，`npm run build` 通过
- Phase 5 最低验证矩阵通过：unit `296 passed`，contract `170 passed`，前端 `131 passed`，`npm run build` 通过
...
- 可复跑 API 边界 probe 通过：`scripts/api_boundary_probe.py` 覆盖 59 个非破坏性 GET 边界，包含越界参数、非法 path/query 类型、非法 path/entity、特殊字符查询，验证预期 422/200、`X-Request-ID`、无 500，且 422 响应带 FastAPI validation detail
- OpenAPI parameter boundary audit 通过：`scripts/openapi_parameter_boundary_audit.py --json-output /tmp/spotify_openapi_parameter_boundary_audit.json` 核算当前 37 个参数边界义务，归属为 36 个 `boundary_probe`、1 个 `controlled_stateful_or_external`，`unaccounted=0`
...
| P3 | API 参数边界覆盖仍分散在少量 contract 测试中 | “空值、超长值、非法值、特殊字符”边界要求缺少一条可独立复跑的只读探针；happy-path API smoke 无法证明代表性 422 响应都带 request id 与 validation detail | `scripts/api_boundary_probe.py` 扩展到 59 个非破坏性 GET 边界：limit/offset/top_n/merge_level/Billboard top-N/week offset/release-cycle window/significance 越界、非法 path/query 类型、invalid/empty entity、特殊字符搜索；新增 unit/contract 护栏，锁定预期 422/200、`X-Request-ID`、无 500 和 FastAPI validation detail |
| P3 | OpenAPI 参数边界义务缺少机器可读归属 | boundary probe 扩展后仍需要确认每个带边界/模式约束的 query 参数、type-only integer query 和 integer path 参数都有明确证据；否则新增参数可能只出现在 `/docs`，没有边界 probe 或显式排除 | 新增 `scripts/openapi_parameter_boundary_audit.py`，读取 `app.openapi()` 并核算 37 个参数边界义务：36 个归属到 `api_boundary_probe.py` case，1 个 stateful POST 参数显式归属为 controlled stateful/external，`unaccounted=0`；`fullstack_verification_check.sh` 已纳入该 audit |
...
- API boundary probe：`scripts/api_boundary_probe.py` 在 TestClient 本地应用实例通过 59/59 个非破坏性 GET 边界，覆盖 Analysis/Dashboard/Leaderboard/Community/Library/Version Merge/Billboard/Release Cycle/Music Entity/Lyrics/Chat/AI Insights 的越界查询参数、非法 path/query 类型、非法 path/entity 与特殊字符搜索；每个响应均验证预期状态、`X-Request-ID`、无 500，422 响应验证 FastAPI validation detail。
- OpenAPI parameter boundary audit：`scripts/openapi_parameter_boundary_audit.py` 直接读取 `backend.main.app.openapi()`，核算 bounded/pattern query 参数、type-only integer query 参数和 integer path 参数的 37 个边界义务，当前结果为 `boundary_probe=36`、`controlled_stateful_or_external=1`、`unaccounted=0`；`--json-output /tmp/spotify_openapi_parameter_boundary_audit.json` 会写出每个 obligation 的 location、name、signature、occurrence_count、category、evidence 与 rationale，后续新增未分类参数边界会使脚本失败。
...
- 全栈聚合 probe：`sh scripts/fullstack_verification_check.sh --backend-url http://127.0.0.1:8000 --frontend-url http://127.0.0.1:5173 --preview-url http://127.0.0.1:4173 --preview-api-url http://127.0.0.1:8000 --web-vitals` 此前在本地后端 8000 + Vite dev 5173 + `vite preview` 4173 环境完整通过，覆盖 backend full `616 passed`、pre-commit、Phase 5 unit `261` / contract `155` / frontend `130` / build、API smoke `91/91`、API boundary `19/19`、Provider error contract、Infrastructure response-model contract、当时默认 route smoke、interaction `5/5`、chart `3/3`、long-list `6/6`、cross-browser Chromium/Firefox/WebKit `PASS`，以及 dev/prod Web Vitals lab；本轮 response-model/API smoke、Billboard enrichment 降级契约、OpenCC 按需加载、路由 smoke 覆盖、preview 转发、route warning 门禁、Web Vitals 预算门禁、runtime resource probe、quickstart timing、OpenAPI operation audit 与 OpenAPI parameter boundary audit 收口后已单独复跑 backend full `666 passed`、Phase 5 unit `296` / contract `170` / frontend `131` / build、OpenAPI operation audit `134 operations / 0 unaccounted`、OpenAPI parameter boundary audit `37 obligations / 0 unaccounted`、API boundary `59/59`、quickstart JSON smoke `2454.6ms`、dev route smoke `38/38`、prod-preview route smoke `38/38`、prod-preview interaction `5/5`、prod-preview cross-browser Chromium/Firefox/WebKit `PASS`、prod-preview Web Vitals `/account` 390px PASS 以及 API smoke `96/96`。`fullstack_verification_check.sh` 当前 dev/preview route smoke 会把 console warning 作为失败，preview route/interaction/chart/long-list/cross-browser smoke 和 Web Vitals 均会通过 `--preview-api-url` 显式分流 `/api` 与 `/covers` 请求，并可用 `--web-vitals-max-*` 把 LCP/CLS/TBT/资源数量/encoded 体积预算纳入聚合矩阵；开启 `--resource-snapshot` 时还会在 benchmark 后采集 backend/frontend/preview 监听进程树 CPU/RSS 快照，并可用 `--resource-max-total-rss-mb` / `--resource-max-total-cpu-percent` 设置聚合资源预算。
...
node scripts/frontend_route_smoke.mjs --base-url http://127.0.0.1:4173 --api-base-url http://127.0.0.1:8000 --viewport both --max-scroll-overflow 0 --fail-on-console-warning
...
- 未逐一实打所有破坏性或外部依赖端点，例如断开 Spotify、导入生产数据、同步远程账号数据等，避免污染本地真实状态；OpenAPI operation audit 已将 134 个 operation 全部归属到 safe GET smoke、targeted contract 或 controlled stateful/external，OpenAPI parameter boundary audit 已将 37 个参数边界义务归属到 boundary probe 或 controlled stateful/external，Settings/Chat mutation 与 Import job 调度已通过 contract 临时环境覆盖。
