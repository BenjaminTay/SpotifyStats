...
def load_yearly_summary_data(_min_ms, _music_only, _merge_enabled):
    conn = get_db()
...
def render():
    df = load_yearly_summary_data(min_ms, music_only, merge_enabled)

    years = sorted(df["ts_year"].unique().tolist(), reverse=True)

    # Controls (inline — was sidebar)
    page_header("🎁 年度总结", description=f"自定义年度回顾 · 最短={min_ms//1000}s")
    ...
    # Seasonal favorites: Spring(3-5), Summer(6-8), Autumn(9-11), Winter(12,1,2)
    seasons = {
        "🌸 春": (3, 4, 5),
        "☀️ 夏": (6, 7, 8),
        "🍂 秋": (9, 10, 11),
    ...
    for season_label, months in seasons.items():
        ...
        if not season_df.empty:
            top_track = season_df.groupby("track_name").size().sort_values(ascending=False).index[0]
            top_count = int(season_df.groupby("track_name").size().sort_values(ascending=False).iloc[0])
            season_tops[season_label] = (top_track, top_count)
        else:
            season_tops[season_label] = ("N/A", 0)
    ...
    with col5:
        season_parts = []
        for season_label, (track_name, count) in season_tops.items():
            season_parts.append(
                f"""<p style="color:#8B7355; margin-bottom:4px;">{season_label}</p>
                <span class="track-name">{track_name}</span>
                <span style="color:#8B7355; font-size:0.85em;"> · {count}次</span>"""
            )
        season_html = "<hr style='border-color:#D4A84B; margin:12px 0;'>".join(season_parts)
        st.markdown(
            f"""<div class="wrapped-card">
            <h3>🍃 四季最爱</h3>
